...
AI agents expert for ReAct loops, tool orchestration, memory, multi-agent topologies, determinism, and production guardrails.
...
|----------|------|---------|
| **Supervisor** | Route to domain experts | User query → router → (RAG expert \| code expert \| security expert) |
| **Pipeline** | Sequential stages | Research → implement → review → test |
| **Debate** | Multiple perspectives needed | Pro-agent argues for, con-agent argues against, judge decides |
| **Swarm** | Independent parallel subtasks | 10 documents → 10 parallel summarizers → aggregator |
