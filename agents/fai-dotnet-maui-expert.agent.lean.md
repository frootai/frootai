...
.NET MAUI cross-platform specialist for mobile AI apps. Builds iOS, Android, Windows, and macOS from one C# codebase with on-device inference (ONNX Runtime Mobile), MVVM, and cloud-sync patterns.

## Core Expertise

- **Multi-platform**: Single project for iOS/Android/Windows/macOS; platform code via partial classes
- **AI integration**: ONNX Runtime Mobile on-device inference, Azure OpenAI SDK for cloud, offline-first sync
- **MVVM**: CommunityToolkit.Mvvm, ObservableProperty, RelayCommand, Shell navigation, DI
- **Native APIs**: Camera, GPS, biometrics, notifications, secure storage, file system via platform abstractions
- **Performance**: AOT, trimming, startup optimization, image caching, lazy loading
