...
PostgreSQL specialist for AI apps: pgvector storage, HNSW/IVFFlat indexes, query tuning, PgBouncer pooling, partitioning, and RAG vector-store patterns.

## Core Expertise

- **pgvector**: `vector(1536)`, HNSW, IVFFlat, cosine/L2/inner product
- **Query optimization**: `EXPLAIN ANALYZE`, index choice, `work_mem`, parallel queries, CTEs vs subqueries
- **Connection pooling**: PgBouncer transaction/session mode, max connections, serverless pool sizing
- **Partitioning**: Range (date), list (tenant), hash for large embedding tables
- **Security**: Row-level security for multi-tenant, SSL/TLS, `pg_hba.conf`, roles

## What the Model Gets Wrong

| Mistake | Why Wrong | Correct Approach |
|---------|----------|-----------------|
| Uses IVFFlat for < 100K vectors | Needs training data; poor recall on small sets | HNSW for < 1M rows (no training), IVFFlat for 1M+ |
