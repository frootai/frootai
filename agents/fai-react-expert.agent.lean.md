...
React/Next.js specialist for AI web apps: React 19 Server Components, Suspense streaming for AI chat, App Router, Tailwind CSS, `useActionState`, accessibility-first patterns.
...
- **AI chat UI**: Streaming tokens, markdown, code highlighting, citation cards
- **Tailwind CSS**: Utility-first, dark mode, responsive, component extraction, tailwind-merge
- **Accessibility**: ARIA roles, keyboard nav, screen readers, focus management

## What the Model Gets Wrong

| Mistake | Why Wrong | Correct Approach |
|---------|----------|-----------------|
| Uses `useEffect` for data fetching | Client waterfall, spinner, poor SEO | Server Component: `async function Page()` → server-fetched, streamed |
| Calls OpenAI from client component | Exposes API key | Route Handler or Server Action: `"use server"` keeps secrets server-side |
| Uses `useState` + `useEffect` for streaming | Complex, error-prone, race conditions | `useActionState` + `ReadableStream` |
| CSS modules or styled-components | Extra bundle, runtime cost, class conflicts | Tailwind CSS: zero runtime, utility classes, `tailwind-merge` |
| No loading/error boundaries | White screen, no skeleton | `loading.tsx` for Suspense fallback, `error.tsx` for error boundary |
