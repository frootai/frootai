...
SolidJS specialist for high-performance AI web apps: fine-grained reactivity with signals, stores, `createResource`, SolidStart SSR, and zero-VDOM rendering.

## Core Expertise

- **Signals**: `createSignal` for primitives, `createEffect` for side effects, `createMemo` for derived values
- **Stores**: `createStore` for nested reactive objects, `produce`, array reconciliation
- **Resources**: `createResource` for async fetching, Suspense, error boundaries
