...
- Implement AG-UI protocol for real-time agent output rendering in frontends
- Support streaming response chunks with typed events: TextDelta, ToolCall, StateSnapshot
- Use Server-Sent Events (SSE) or WebSocket for real-time communication
- Define UI component schemas: ChatMessage, CodeBlock, Chart, Table, Form, ActionButton
- Support progressive rendering: show partial results while agent is still processing

### Event Types
- **TextDelta**: Incremental text tokens for streaming chat responses
- **ToolCallStart/ToolCallEnd**: Show tool execution status in UI (spinner, progress bar)
- **StateSnapshot**: Full agent state for resumable conversations
- **MetadataUpdate**: Update context panel (tokens used, model, latency, cost)
- **ActionRequest**: Agent requests user input via structured form
- **UIComponentRender**: Agent emits a UI component (chart, table, code editor)

### Frontend Integration
- React hooks: `useAGUIStream(agentUrl)` for SSE consumption
- Vue composable: `useAgentStream()` with reactive state binding
- Next.js: Server component streaming with Suspense boundaries
- Mobile: React Native bridge for native rendering of AG-UI events
- Accessibility: All emitted components must include ARIA labels and keyboard navigation

### FrootAI Integration
- AG-UI handles rendering, FAI Protocol handles wiring, MCP handles tools
- Agent primitives (.agent.md) define which UI components they can emit
- Solution plays configure AG-UI endpoints in fai-manifest.json
- Chatbot page on frootai.dev uses AG-UI for streaming responses
- VS Code extension renders AG-UI events in Copilot Chat webview
