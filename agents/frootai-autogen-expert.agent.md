...
### Core Implementation
- Design autogen solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across autogen service boundaries using correlation IDs
- Use structured configuration management — all autogen parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all autogen service integrations

### Autogen Integration with Azure
- Use Azure Managed Identity for authentication to autogen services (never API keys in code)
- Deploy autogen components using Bicep IaC with parameterized templates
- Configure Azure Monitor for autogen-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for autogen services in production environments
...
### Design Principles
- **Modularity**: Build autogen components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every autogen operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test autogen logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate autogen service interactions with test environment
- **Load tests**: Verify autogen performance under expected peak load (2x normal)
...
## Autogen FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate autogen with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-autogen-expert.agent.md"],
    "instructions": ["../../instructions/autogen.instructions.md"]
  }
}
```

**Q: What WAF pillars does autogen align with?**
Primary pillars: reliability, responsible-ai. Every autogen implementation should include:
...
**Q: How do I evaluate autogen quality?**
Use the play's eval.py with autogen-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the autogen operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot autogen failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for autogen-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the autogen endpoint directly
