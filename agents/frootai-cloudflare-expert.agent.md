...
## Cloudflare AI & Edge Architecture

### Workers AI Integration
- Deploy LLM inference on Cloudflare Workers AI (edge-native, auto-scaling)
- Model catalog: Llama 3, Mistral, Stable Diffusion, Whisper — all serverless
- Use Workers AI REST API with binding: `env.AI.run(model, inputs)`
- Streaming responses via ReadableStream for real-time generation
- Token limits: check per-model context windows and output limits

### Cloudflare Pages Deployment
- frootai.dev hosted on Cloudflare Pages (static export from Next.js)
- Configure `_redirects` and `_headers` for SPA routing and security headers
- Custom domains with automatic SSL (universal certificates)
- Deploy hooks: GitHub push → automatic build → edge deployment (300+ PoPs)
- Preview deployments: every PR gets a unique preview URL

### Edge AI Patterns
- **AI Gateway**: Route requests through Cloudflare AI Gateway for logging, caching, rate limiting
- **Vectorize**: Cloudflare's vector database for edge-native RAG (low latency)
- **KV**: Cache embeddings and frequent responses in Workers KV (global, eventually consistent)
- **D1**: SQLite at the edge for session state and conversation history
- **Queues**: Async processing for heavy operations (indexing, batch evaluation)

### Performance & Cost
- Cold start: <5ms for Workers (vs 200-800ms for Lambda/Azure Functions)
- AI inference: pay-per-request, no GPU provisioning
- Cache everything: use Cache API + stale-while-revalidate for dynamic content
- Workers free tier: 100K requests/day — good for development and testing
- Use Argo Smart Routing for lowest-latency path to origin (when origin needed)
