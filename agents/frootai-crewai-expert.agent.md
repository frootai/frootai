...
### Core Implementation
- Design crewai solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across crewai service boundaries using correlation IDs
- Use structured configuration management — all crewai parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all crewai service integrations

### Crewai Integration with Azure
- Use Azure Managed Identity for authentication to crewai services (never API keys in code)
- Deploy crewai components using Bicep IaC with parameterized templates
- Configure Azure Monitor for crewai-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for crewai services in production environments
...
### Design Principles
- **Modularity**: Build crewai components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every crewai operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test crewai logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate crewai service interactions with test environment
- **Load tests**: Verify crewai performance under expected peak load (2x normal)
...
## Crewai FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate crewai with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-crewai-expert.agent.md"],
    "instructions": ["../../instructions/crewai.instructions.md"]
  }
}
```

**Q: What WAF pillars does crewai align with?**
Primary pillars: reliability, operational-excellence. Every crewai implementation should include:
...
**Q: How do I evaluate crewai quality?**
Use the play's eval.py with crewai-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the crewai operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot crewai failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for crewai-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the crewai endpoint directly
