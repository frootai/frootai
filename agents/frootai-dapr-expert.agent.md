...
## Dapr Architecture Patterns

### Core Implementation
- Design dapr solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across dapr service boundaries using correlation IDs
- Use structured configuration management — all dapr parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all dapr service integrations

### Dapr Integration with Azure
- Use Azure Managed Identity for authentication to dapr services (never API keys in code)
- Deploy dapr components using Bicep IaC with parameterized templates
- Configure Azure Monitor for dapr-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for dapr services in production environments
...
### Design Principles
- **Modularity**: Build dapr components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every dapr operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test dapr logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate dapr service interactions with test environment
- **Load tests**: Verify dapr performance under expected peak load (2x normal)
...
## Dapr FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate dapr with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-dapr-expert.agent.md"],
    "instructions": ["../../instructions/dapr.instructions.md"]
  }
}
```

**Q: What WAF pillars does dapr align with?**
Primary pillars: reliability, operational-excellence. Every dapr implementation should include:
...
**Q: How do I evaluate dapr quality?**
Use the play's eval.py with dapr-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the dapr operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot dapr failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for dapr-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the dapr endpoint directly
