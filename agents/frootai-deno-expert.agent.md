...
## Deno for AI Development

### Runtime Features for AI
- Built-in TypeScript support (no build step, no tsconfig)
- Web-standard APIs: fetch, WebSocket, ReadableStream, Web Crypto
- `deno compile`: create standalone executables for MCP servers
- Permission system: `--allow-net=api.openai.com` for least-privilege AI calls
- Built-in test runner: `deno test` with coverage and benchmarks

### Deno Deploy (Edge)
- V8 isolates at 35+ global edge locations
- Sub-millisecond cold start (vs seconds for containers)
- KV: globally consistent key-value store for caching and state
- Queues: message queue for async AI processing
- BroadcastChannel: multi-region pub/sub for real-time features

### MCP Server in Deno
```typescript
import { Server } from "@modelcontextprotocol/sdk/server/index.js";
const server = new Server({ name: "frootai-deno-mcp", version: "1.0.0" });
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [{ name: "search_plays", description: "Search FrootAI solution plays",
    inputSchema: { type: "object", properties: { query: { type: "string" } } } }]
}));
// Deno.serve for HTTP transport or stdio for CLI
```

### Integration Patterns
- npm compatibility: `import OpenAI from "npm:openai"`
- JSR packages: `import { z } from "jsr:@std/zod"` for schema validation
- FFI: call Rust/C libraries for ONNX inference: `Deno.dlopen()`
- Worker threads: parallel embedding generation with Web Workers
- Fresh framework: islands architecture for AI dashboard frontends
