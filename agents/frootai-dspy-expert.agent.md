...
## Dspy Architecture Patterns

### Core Implementation
- Design dspy solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across dspy service boundaries using correlation IDs
- Use structured configuration management — all dspy parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all dspy service integrations

### Dspy Integration with Azure
- Use Azure Managed Identity for authentication to dspy services (never API keys in code)
- Deploy dspy components using Bicep IaC with parameterized templates
- Configure Azure Monitor for dspy-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for dspy services in production environments
...
### Design Principles
- **Modularity**: Build dspy components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every dspy operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test dspy logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate dspy service interactions with test environment
- **Load tests**: Verify dspy performance under expected peak load (2x normal)
...
## Dspy FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate dspy with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-dspy-expert.agent.md"],
    "instructions": ["../../instructions/dspy.instructions.md"]
  }
}
```

**Q: What WAF pillars does dspy align with?**
Primary pillars: performance-efficiency, cost-optimization. Every dspy implementation should include:
...
**Q: How do I evaluate dspy quality?**
Use the play's eval.py with dspy-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the dspy operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot dspy failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for dspy-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the dspy endpoint directly
