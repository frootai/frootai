...
### Core Implementation
- Design grpc solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across grpc service boundaries using correlation IDs
- Use structured configuration management — all grpc parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all grpc service integrations

### Grpc Integration with Azure
- Use Azure Managed Identity for authentication to grpc services (never API keys in code)
- Deploy grpc components using Bicep IaC with parameterized templates
- Configure Azure Monitor for grpc-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for grpc services in production environments
...
### Design Principles
- **Modularity**: Build grpc components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every grpc operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test grpc logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate grpc service interactions with test environment
- **Load tests**: Verify grpc performance under expected peak load (2x normal)
...
## Grpc FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate grpc with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-grpc-expert.agent.md"],
    "instructions": ["../../instructions/grpc.instructions.md"]
  }
}
```

**Q: What WAF pillars does grpc align with?**
Primary pillars: performance-efficiency, reliability. Every grpc implementation should include:
...
**Q: How do I evaluate grpc quality?**
Use the play's eval.py with grpc-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the grpc operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot grpc failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for grpc-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the grpc endpoint directly
