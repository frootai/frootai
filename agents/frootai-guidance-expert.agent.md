...
### Core Implementation
- Design guidance solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across guidance service boundaries using correlation IDs
- Use structured configuration management — all guidance parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all guidance service integrations

### Guidance Integration with Azure
- Use Azure Managed Identity for authentication to guidance services (never API keys in code)
- Deploy guidance components using Bicep IaC with parameterized templates
- Configure Azure Monitor for guidance-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for guidance services in production environments
...
### Design Principles
- **Modularity**: Build guidance components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every guidance operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test guidance logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate guidance service interactions with test environment
- **Load tests**: Verify guidance performance under expected peak load (2x normal)
...
## Guidance FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate guidance with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-guidance-expert.agent.md"],
    "instructions": ["../../instructions/guidance.instructions.md"]
  }
}
```

**Q: What WAF pillars does guidance align with?**
Primary pillars: reliability, performance-efficiency. Every guidance implementation should include:
...
**Q: How do I evaluate guidance quality?**
Use the play's eval.py with guidance-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the guidance operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot guidance failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for guidance-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the guidance endpoint directly
