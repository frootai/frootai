...
### Core Implementation
- Design htmx solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across htmx service boundaries using correlation IDs
- Use structured configuration management — all htmx parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all htmx service integrations

### Htmx Integration with Azure
- Use Azure Managed Identity for authentication to htmx services (never API keys in code)
- Deploy htmx components using Bicep IaC with parameterized templates
- Configure Azure Monitor for htmx-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for htmx services in production environments
...
### Design Principles
- **Modularity**: Build htmx components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every htmx operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test htmx logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate htmx service interactions with test environment
- **Load tests**: Verify htmx performance under expected peak load (2x normal)
...
## Htmx FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate htmx with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-htmx-expert.agent.md"],
    "instructions": ["../../instructions/htmx.instructions.md"]
  }
}
```

**Q: What WAF pillars does htmx align with?**
Primary pillars: performance-efficiency, reliability. Every htmx implementation should include:
...
**Q: How do I evaluate htmx quality?**
Use the play's eval.py with htmx-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the htmx operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot htmx failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for htmx-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the htmx endpoint directly
