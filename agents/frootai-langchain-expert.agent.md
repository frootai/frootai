...
### Core Implementation
- Design langchain solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across langchain service boundaries using correlation IDs
- Use structured configuration management — all langchain parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all langchain service integrations

### Langchain Integration with Azure
- Use Azure Managed Identity for authentication to langchain services (never API keys in code)
- Deploy langchain components using Bicep IaC with parameterized templates
- Configure Azure Monitor for langchain-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for langchain services in production environments
...
### Design Principles
- **Modularity**: Build langchain components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every langchain operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test langchain logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate langchain service interactions with test environment
- **Load tests**: Verify langchain performance under expected peak load (2x normal)
...
## Langchain FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate langchain with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-langchain-expert.agent.md"],
    "instructions": ["../../instructions/langchain.instructions.md"]
  }
}
```

**Q: What WAF pillars does langchain align with?**
Primary pillars: reliability, operational-excellence. Every langchain implementation should include:
...
**Q: How do I evaluate langchain quality?**
Use the play's eval.py with langchain-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the langchain operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot langchain failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for langchain-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the langchain endpoint directly
