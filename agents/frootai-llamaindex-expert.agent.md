...
## Llamaindex Architecture Patterns

### Core Implementation
- Design llamaindex solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across llamaindex service boundaries using correlation IDs
- Use structured configuration management — all llamaindex parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all llamaindex service integrations

### Llamaindex Integration with Azure
- Use Azure Managed Identity for authentication to llamaindex services (never API keys in code)
- Deploy llamaindex components using Bicep IaC with parameterized templates
- Configure Azure Monitor for llamaindex-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for llamaindex services in production environments
...
### Design Principles
- **Modularity**: Build llamaindex components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every llamaindex operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test llamaindex logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate llamaindex service interactions with test environment
- **Load tests**: Verify llamaindex performance under expected peak load (2x normal)
...
## Llamaindex FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate llamaindex with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-llamaindex-expert.agent.md"],
    "instructions": ["../../instructions/llamaindex.instructions.md"]
  }
}
```

**Q: What WAF pillars does llamaindex align with?**
Primary pillars: performance-efficiency, reliability. Every llamaindex implementation should include:
...
**Q: How do I evaluate llamaindex quality?**
Use the play's eval.py with llamaindex-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the llamaindex operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot llamaindex failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for llamaindex-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the llamaindex endpoint directly
