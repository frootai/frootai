...
### Core Implementation
- Design nats solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across nats service boundaries using correlation IDs
- Use structured configuration management — all nats parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all nats service integrations

### Nats Integration with Azure
- Use Azure Managed Identity for authentication to nats services (never API keys in code)
- Deploy nats components using Bicep IaC with parameterized templates
- Configure Azure Monitor for nats-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for nats services in production environments
...
### Design Principles
- **Modularity**: Build nats components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every nats operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test nats logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate nats service interactions with test environment
- **Load tests**: Verify nats performance under expected peak load (2x normal)
...
## Nats FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate nats with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-nats-expert.agent.md"],
    "instructions": ["../../instructions/nats.instructions.md"]
  }
}
```

**Q: What WAF pillars does nats align with?**
Primary pillars: reliability, performance-efficiency. Every nats implementation should include:
...
**Q: How do I evaluate nats quality?**
Use the play's eval.py with nats-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the nats operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot nats failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for nats-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the nats endpoint directly
