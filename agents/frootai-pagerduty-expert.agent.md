...
### Core Implementation
- Design pagerduty solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across pagerduty service boundaries using correlation IDs
- Use structured configuration management — all pagerduty parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all pagerduty service integrations

### Pagerduty Integration with Azure
- Use Azure Managed Identity for authentication to pagerduty services (never API keys in code)
- Deploy pagerduty components using Bicep IaC with parameterized templates
- Configure Azure Monitor for pagerduty-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for pagerduty services in production environments
...
### Design Principles
- **Modularity**: Build pagerduty components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every pagerduty operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test pagerduty logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate pagerduty service interactions with test environment
- **Load tests**: Verify pagerduty performance under expected peak load (2x normal)
...
## Pagerduty FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate pagerduty with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-pagerduty-expert.agent.md"],
    "instructions": ["../../instructions/pagerduty.instructions.md"]
  }
}
```

**Q: What WAF pillars does pagerduty align with?**
Primary pillars: reliability, security, operational-excellence. Every pagerduty implementation should include:
...
**Q: How do I evaluate pagerduty quality?**
Use the play's eval.py with pagerduty-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the pagerduty operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot pagerduty failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for pagerduty-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the pagerduty endpoint directly
