...
## Protobuf Architecture Patterns

### Core Implementation
- Design protobuf solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across protobuf service boundaries using correlation IDs
- Use structured configuration management — all protobuf parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all protobuf service integrations

### Protobuf Integration with Azure
- Use Azure Managed Identity for authentication to protobuf services (never API keys in code)
- Deploy protobuf components using Bicep IaC with parameterized templates
- Configure Azure Monitor for protobuf-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for protobuf services in production environments
...
### Design Principles
- **Modularity**: Build protobuf components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every protobuf operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test protobuf logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate protobuf service interactions with test environment
- **Load tests**: Verify protobuf performance under expected peak load (2x normal)
...
## Protobuf FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate protobuf with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-protobuf-expert.agent.md"],
    "instructions": ["../../instructions/protobuf.instructions.md"]
  }
}
```

**Q: What WAF pillars does protobuf align with?**
Primary pillars: performance-efficiency, operational-excellence. Every protobuf implementation should include:
...
**Q: How do I evaluate protobuf quality?**
Use the play's eval.py with protobuf-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the protobuf operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot protobuf failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for protobuf-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the protobuf endpoint directly
