...
## Ray Architecture Patterns

### Core Implementation
- Design ray solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across ray service boundaries using correlation IDs
- Use structured configuration management — all ray parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all ray service integrations

### Ray Integration with Azure
- Use Azure Managed Identity for authentication to ray services (never API keys in code)
- Deploy ray components using Bicep IaC with parameterized templates
- Configure Azure Monitor for ray-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for ray services in production environments
...
### Design Principles
- **Modularity**: Build ray components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every ray operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test ray logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate ray service interactions with test environment
- **Load tests**: Verify ray performance under expected peak load (2x normal)
...
## Ray FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate ray with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-ray-expert.agent.md"],
    "instructions": ["../../instructions/ray.instructions.md"]
  }
}
```

**Q: What WAF pillars does ray align with?**
Primary pillars: performance-efficiency, cost-optimization. Every ray implementation should include:
...
**Q: How do I evaluate ray quality?**
Use the play's eval.py with ray-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the ray operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot ray failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for ray-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the ray endpoint directly
