...
## Servicenow Architecture Patterns

### Core Implementation
- Design servicenow solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across servicenow service boundaries using correlation IDs
- Use structured configuration management — all servicenow parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all servicenow service integrations

### Servicenow Integration with Azure
- Use Azure Managed Identity for authentication to servicenow services (never API keys in code)
- Deploy servicenow components using Bicep IaC with parameterized templates
- Configure Azure Monitor for servicenow-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for servicenow services in production environments
...
### Design Principles
- **Modularity**: Build servicenow components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every servicenow operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test servicenow logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate servicenow service interactions with test environment
- **Load tests**: Verify servicenow performance under expected peak load (2x normal)
...
## Servicenow FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate servicenow with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-servicenow-expert.agent.md"],
    "instructions": ["../../instructions/servicenow.instructions.md"]
  }
}
```

**Q: What WAF pillars does servicenow align with?**
Primary pillars: reliability, security, operational-excellence. Every servicenow implementation should include:
...
**Q: How do I evaluate servicenow quality?**
Use the play's eval.py with servicenow-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the servicenow operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot servicenow failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for servicenow-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the servicenow endpoint directly
