...
### Core Implementation
- Design slack solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across slack service boundaries using correlation IDs
- Use structured configuration management — all slack parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all slack service integrations

### Slack Integration with Azure
- Use Azure Managed Identity for authentication to slack services (never API keys in code)
- Deploy slack components using Bicep IaC with parameterized templates
- Configure Azure Monitor for slack-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for slack services in production environments
...
### Design Principles
- **Modularity**: Build slack components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every slack operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test slack logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate slack service interactions with test environment
- **Load tests**: Verify slack performance under expected peak load (2x normal)
...
## Slack FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate slack with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-slack-expert.agent.md"],
    "instructions": ["../../instructions/slack.instructions.md"]
  }
}
```

**Q: What WAF pillars does slack align with?**
Primary pillars: reliability, security, operational-excellence. Every slack implementation should include:
...
**Q: How do I evaluate slack quality?**
Use the play's eval.py with slack-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the slack operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot slack failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for slack-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the slack endpoint directly
