...
### Core Implementation
- Design solid solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across solid service boundaries using correlation IDs
- Use structured configuration management — all solid parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all solid service integrations

### Solid Integration with Azure
- Use Azure Managed Identity for authentication to solid services (never API keys in code)
- Deploy solid components using Bicep IaC with parameterized templates
- Configure Azure Monitor for solid-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for solid services in production environments
...
### Design Principles
- **Modularity**: Build solid components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every solid operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test solid logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate solid service interactions with test environment
- **Load tests**: Verify solid performance under expected peak load (2x normal)
...
## Solid FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate solid with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-solid-expert.agent.md"],
    "instructions": ["../../instructions/solid.instructions.md"]
  }
}
```

**Q: What WAF pillars does solid align with?**
Primary pillars: performance-efficiency, reliability. Every solid implementation should include:
...
**Q: How do I evaluate solid quality?**
Use the play's eval.py with solid-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the solid operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot solid failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for solid-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the solid endpoint directly
