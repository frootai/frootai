...
## Teams Architecture Patterns

### Core Implementation
- Design teams solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across teams service boundaries using correlation IDs
- Use structured configuration management — all teams parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all teams service integrations

### Teams Integration with Azure
- Use Azure Managed Identity for authentication to teams services (never API keys in code)
- Deploy teams components using Bicep IaC with parameterized templates
- Configure Azure Monitor for teams-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for teams services in production environments
...
### Design Principles
- **Modularity**: Build teams components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every teams operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test teams logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate teams service interactions with test environment
- **Load tests**: Verify teams performance under expected peak load (2x normal)
...
## Teams FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate teams with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-teams-expert.agent.md"],
    "instructions": ["../../instructions/teams.instructions.md"]
  }
}
```

**Q: What WAF pillars does teams align with?**
Primary pillars: reliability, security, operational-excellence. Every teams implementation should include:
...
**Q: How do I evaluate teams quality?**
Use the play's eval.py with teams-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the teams operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot teams failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for teams-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the teams endpoint directly
