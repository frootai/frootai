...
### Core Implementation
- Design temporal solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across temporal service boundaries using correlation IDs
- Use structured configuration management — all temporal parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all temporal service integrations

### Temporal Integration with Azure
- Use Azure Managed Identity for authentication to temporal services (never API keys in code)
- Deploy temporal components using Bicep IaC with parameterized templates
- Configure Azure Monitor for temporal-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for temporal services in production environments
...
### Design Principles
- **Modularity**: Build temporal components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every temporal operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test temporal logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate temporal service interactions with test environment
- **Load tests**: Verify temporal performance under expected peak load (2x normal)
...
## Temporal FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate temporal with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-temporal-expert.agent.md"],
    "instructions": ["../../instructions/temporal.instructions.md"]
  }
}
```

**Q: What WAF pillars does temporal align with?**
Primary pillars: reliability, operational-excellence. Every temporal implementation should include:
...
**Q: How do I evaluate temporal quality?**
Use the play's eval.py with temporal-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the temporal operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot temporal failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for temporal-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the temporal endpoint directly
