...
## Turso Edge Database for AI

### libSQL Architecture
- Turso is built on libSQL (SQLite fork) with server mode and replication
- Embedded replicas: local SQLite file synced with remote Turso database
- Edge deployment: replicas at 30+ global locations for <10ms reads
- Schema: standard SQLite with vector search extension
- WASM compatibility: runs in browser and edge runtimes

### Vector Search Extension
- Enable vector columns: `CREATE TABLE docs (id TEXT, content TEXT, embedding F32_BLOB(1536))`
- Vector index: `CREATE INDEX docs_idx ON docs(libsql_vector_idx(embedding))`
- Search: `SELECT * FROM vector_top_k('docs_idx', query_vector, 10)`
- Hybrid search: combine FTS5 full-text + vector similarity in single query
- Quantization: F16 storage for 50% space reduction

### Embedded Replicas for AI
- Local inference apps: embed Turso replica alongside local LLM
- Offline-first: works without network, syncs when reconnected
- Edge functions: co-locate database with compute for sub-millisecond queries
- Mobile apps: SQLite-compatible, works with expo-sqlite and capacitor
- Sync frequency: configurable (1s for real-time, 60s for batch workloads)

### Cost Model
- Free tier: 500 databases, 9GB total storage, 500M rows read/month
- Pro tier: per-read/write pricing (ideal for bursty AI workloads)
- No cold start: embedded replicas are always warm
- Storage: columnar compression reduces embedding storage by 40%
