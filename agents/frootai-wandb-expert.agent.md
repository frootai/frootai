...
## Wandb Architecture Patterns

### Core Implementation
- Design wandb solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across wandb service boundaries using correlation IDs
- Use structured configuration management — all wandb parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all wandb service integrations

### Wandb Integration with Azure
- Use Azure Managed Identity for authentication to wandb services (never API keys in code)
- Deploy wandb components using Bicep IaC with parameterized templates
- Configure Azure Monitor for wandb-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for wandb services in production environments
...
### Design Principles
- **Modularity**: Build wandb components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every wandb operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test wandb logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate wandb service interactions with test environment
- **Load tests**: Verify wandb performance under expected peak load (2x normal)
...
## Wandb FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate wandb with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-wandb-expert.agent.md"],
    "instructions": ["../../instructions/wandb.instructions.md"]
  }
}
```

**Q: What WAF pillars does wandb align with?**
Primary pillars: operational-excellence, performance-efficiency. Every wandb implementation should include:
...
**Q: How do I evaluate wandb quality?**
Use the play's eval.py with wandb-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the wandb operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot wandb failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for wandb-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the wandb endpoint directly
