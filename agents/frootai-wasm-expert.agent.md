...
## Wasm Architecture Patterns

### Core Implementation
- Design wasm solutions for high availability with automatic failover and health monitoring
- Implement distributed tracing across wasm service boundaries using correlation IDs
- Use structured configuration management — all wasm parameters in config/*.json
- Apply defensive coding: validate all inputs at system boundaries, handle partial failures gracefully
- Follow the principle of least privilege for all wasm service integrations

### Wasm Integration with Azure
- Use Azure Managed Identity for authentication to wasm services (never API keys in code)
- Deploy wasm components using Bicep IaC with parameterized templates
- Configure Azure Monitor for wasm-specific metrics and alerting
- Use Azure Key Vault for all secrets, connection strings, and API keys
- Implement private endpoints for wasm services in production environments
...
### Design Principles
- **Modularity**: Build wasm components as reusable FAI primitives (skills, hooks, plugins)
- **Observability**: Emit structured telemetry for every wasm operation (latency, errors, throughput)
...
### Testing Strategy
- **Unit tests**: Test wasm logic with mocked dependencies (>80% coverage)
- **Integration tests**: Validate wasm service interactions with test environment
- **Load tests**: Verify wasm performance under expected peak load (2x normal)
...
## Wasm FAQ & Troubleshooting

### Common Issues

**Q: How do I integrate wasm with FrootAI solution plays?**
Wire the agent via fai-manifest.json:
```json
{
  "primitives": {
    "agents": ["../../agents/frootai-wasm-expert.agent.md"],
    "instructions": ["../../instructions/wasm.instructions.md"]
  }
}
```

**Q: What WAF pillars does wasm align with?**
Primary pillars: performance-efficiency, security. Every wasm implementation should include:
...
**Q: How do I evaluate wasm quality?**
Use the play's eval.py with wasm-specific metrics:
- Relevance: does the output match the expected behavior?
- Latency: p95 < threshold for the wasm operation
- Error rate: < 1% for production workloads
- Cost: per-operation cost within budget

**Q: How do I troubleshoot wasm failures?**
1. Check Application Insights for error traces with correlation ID
2. Review structured logs for wasm-specific error codes
3. Verify configuration in config/*.json (model, endpoint, parameters)
4. Test with curl/Postman against the wasm endpoint directly
