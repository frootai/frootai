// Generated by scripts/generate-agent-semantic-runtime.js. Do not edit.
// T006 source: bbd9002f5bbaaa87ce08bc4c06c0b8d071c35640:scripts/validate-agent-fai-contracts.cjs
// T006 Git blob: d8e30e0fd993371fd8f3c2de196dba76c5c9af50
// Incremental adapter SHA-256: 6a849b058a5e58802e351759618a667615df8cc7e8c5926edfc157c320ec1645
// @ts-check
"use strict";

const EVENT_FIELDS = Object.freeze({
  "session.started": ["eventType", "surface"],
  "session.resumed": ["eventType", "surface"],
  "turn.accepted": ["eventType", "intent", "authority", "principalDigest", "manifestDigest", "policyVersion"],
  "turn.completed": ["eventType", "outcome", "artifactRefs"],
  "turn.failed": ["eventType", "errorId"],
  "turn.cancelled": ["eventType", "reason", "initiator", "modelDisposition"],
  "context.accepted": ["eventType", "manifestId", "manifestDigest", "reasonCode"],
  "context.rejected": ["eventType", "manifestId", "manifestDigest", "reasonCode"],
  "context.compacted": ["eventType", "manifestId", "manifestDigest", "reasonCode"],
  "retrieval.started": ["eventType", "queryDigest"],
  "retrieval.source": ["eventType", "sourceId", "category", "href", "rank"],
  "retrieval.completed": ["eventType", "sourceCount"],
  "retrieval.failed": ["eventType", "errorId", "sourceCount"],
  "retrieval.cancelled": ["eventType", "reason", "sourceCount"],
  "tool.proposed": ["eventType", "toolCallId", "toolName", "actionId", "profileRef", "risk", "inputDigest"],
  "policy.decided": ["eventType", "toolCallId", "decision", "policyVersion", "reasonCode"],
  "approval.required": ["eventType", "toolCallId", "toolName", "actionId", "profileRef", "risk", "approvalContract", "bindings", "bindingValuesDigest", "principalDigest", "manifestDigest", "inputDigest", "policyVersion", "egress", "expiresAt"],
  "approval.granted": ["eventType", "approvalId", "toolCallId", "toolName", "actionId", "profileRef", "risk", "approvalContract", "bindings", "bindingValuesDigest", "principalDigest", "manifestDigest", "inputDigest", "policyVersion", "egress", "expiresAt"],
  "tool.started": ["eventType", "toolCallId"],
  "tool.progress": ["eventType", "toolCallId", "progressPercent", "messageCode"],
  "tool.completed": ["eventType", "toolCallId", "outputDigest", "effectId"],
  "tool.failed": ["eventType", "toolCallId", "errorId"],
  "model.started": ["eventType", "modelAlias", "stream"],
  "model.delta": ["eventType", "content"],
  "model.completed": ["eventType", "finishReason"],
  "model.failed": ["eventType", "errorId"],
  "artifact.created": ["eventType", "artifactId", "digest"],
  "evidence.created": ["eventType", "evidenceId", "digest"],
  "usage.receipt": ["eventType", "receiptId"],
  warning: ["eventType", "code", "messageCode"],
  limitation: ["eventType", "code", "messageCode"],
});
...
function assertUnicodeScalarString(value) {
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (codeUnit >= 0xd800 && codeUnit <= 0xdbff) {
      ...
      if (!(next >= 0xdc00 && next <= 0xdfff)) throw new TypeError("lone high surrogate in canonical string");
      index += 1;
    } else if (codeUnit >= 0xdc00 && codeUnit <= 0xdfff) {
      throw new TypeError("lone low surrogate in canonical string");
    }
  }
}
...
function replayReduceEvents(events) {
  const errors = [];
  ...
  for (const event of events) {
    ...
    if (identities.has(event.eventId)) {
      if (identities.get(event.eventId) !== serialized) errors.push("stream-event-id-collision");
      continue;
    }
    identities.set(event.eventId, serialized);
    reduced.push(event);
  }
  return { events: reduced, errors };
}

function validateTimestamps(value, prefix = "") {
  const errors = [];
  if (!value || typeof value !== "object") return errors;
  for (const [key, item] of Object.entries(value)) {
    const path = `${prefix}/${key}`;
    if (item !== null && key.endsWith("At") && (typeof item !== "string" || !Number.isFinite(Date.parse(item)))) errors.push(`timestamp-invalid:${path}`);
    else if (typeof item === "object") errors.push(...validateTimestamps(item, path));
  }
  return errors;
}

function validateEvent(event) {
  const expected = EVENT_FIELDS[event.type];
  if (!expected) return ["event-type-unknown"];
  const errors = validateTimestamps(event);
  if (event.data.eventType !== event.type) errors.push("event-payload-type-mismatch");
  const optionalFields = event.data.actionId === "ACT-004" ? ["prerequisiteApprovalId", "consentDigest"] : [];
  if (JSON.stringify(Object.keys(event.data).sort()) !== JSON.stringify([...expected, ...optionalFields].sort())) errors.push("event-payload-fields");
  ...
  if (["approval.required", "approval.granted"].includes(event.type)) {
    ...
    if (!action || JSON.stringify([...event.data.bindings].sort()) !== JSON.stringify([...action.bindings].sort())) errors.push("event-approval-bindings");
...
function validateEventStream(events) {
  const replay = replayReduceEvents(events);
  const errors = [...replay.errors];
  const replayReducedEvents = replay.events;
  ...
  let retrievalOutcome = null;
  let cancellationObserved = false;
  let lifecycleFailureErrorId = null;
  let sessionLifecycleSeen = false;
  let turnActivitySeen = false;
  const first = replayReducedEvents[0];
  for (const event of replayReducedEvents) {
    ...
    if (event.type === "approval.granted") {
      ...
      if (requirement && JSON.stringify([...requirement.bindings].sort()) !== JSON.stringify([...event.data.bindings].sort())) errors.push("stream-grant-bindings-mismatch");
    ...
    if (event.type === "model.completed") {
      ...
      modelActive = false;
      modelFinishReason = event.data.finishReason;
    ...
    if (["retrieval.failed", "retrieval.cancelled"].includes(event.type)) {
      ...
      retrievalActive = false;
      retrievalOutcome = event.type === "retrieval.failed" ? "failed" : "cancelled";
    ...
    if (TERMINAL_EVENTS.has(event.type)) {
      ...
      if (event.type === "turn.cancelled") {
        const dispositionMatches = (event.data.modelDisposition === "not-started" && !modelStarted)
        ...
        if (!dispositionMatches) errors.push("stream-cancelled-model-disposition");
      }
      terminalSeen = true;
    }
