...
An agent definition (`.agent.md`) that declares `mcpAttachments` — specifying which external MCP server areas should be attached when the agent runs. The engine reads these declarations, attaches the correct servers, and exposes their tools to the agent's context at runtime.

## Why Declare MCP Attachments?

Without explicit declarations, the engine doesn't know which external tools your agent needs. Declaring `mcpAttachments` provides:

| Benefit | How It Helps |
|---------|-------------|
| **Deterministic context** | The engine attaches exactly the tools you need — no more, no less |
| **CI validation** | The M10.5 validator catches typos and references to non-existent areas before deploy |
| **Trust enforcement** | The M10.6 trust policy validator blocks untrusted community servers unless explicitly overridden |
| **Reproducibility** | Any collaborator running the same agent gets the same federated tool surface |
...
The `mcpAttachments` field supports two shapes:
...
All listed areas are treated as optional — the agent can run without them but gains extra tools when they're available.
...
- **required** — the agent aborts at pre-flight if these areas cannot be attached
- **optional** — the agent runs with degraded capability if these are unavailable
...
Treated identically to the bare array form (all optional).
...
Open the agent file and add the `mcpAttachments` field to the YAML frontmatter block:
...
If a slug is misspelled or refers to an area not in the roster:
...
The agent now has access to all tools from the Azure and GitHub MCP servers in addition to its standard `codebase` and `terminal` tools.
...
The `trustOverrides` value is a free-form reason string documenting why the community server is acceptable.
