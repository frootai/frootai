...
A FrootAI plugin that ships its own MCP server — declared via the `providesMcp` field in `plugin.json`. Once published, other agents and plays can reference this server by slug in their `mcpAttachments` or `mcp_scope.attached` declarations, and the engine will auto-discover and attach it.

## Why providesMcp?

Most plugins consume external MCP tools. A `providesMcp` plugin is different — it *creates* a new tool surface that others can attach to:
...
Provider plugins appear in the marketplace with a **federation-ready** badge and are auto-registered in the federated areas catalog on install.
...
The `command` + `args` in `providesMcp` must resolve to a working server when the plugin is installed.
...
If your server should be discoverable by other plays/agents across the ecosystem, add it to the canonical roster:
...
Now other agents can reference `"my-kb"` in their `mcpAttachments` and the M10.5 validator will accept it.
...
The publish command validates the `providesMcp` schema, builds a dry-run payload, and reports what would be submitted to the marketplace API:
...
If your server runs as a remote HTTP service instead of a local subprocess:
...
Your `trust` claim is *self-declared* — the engine re-evaluates it against the shipped trust manifest. Claiming `first-party-ms` for a non-Microsoft server won't grant trust; it will be downgraded at verification time.
