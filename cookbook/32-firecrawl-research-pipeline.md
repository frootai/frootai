...
> Compose three MCP servers into one deep-research pipeline: crawl + scrape the
> target sites with **Firecrawl**, broaden coverage with live web search from
> **Tavily**, and ground every technical claim against version-pinned docs with
> **Context7** — synthesizing a multi-source research brief from one FrootAI play.

## What You'll Build

A solution play whose agent attaches three MCP servers and runs a
**crawl → broaden → ground → synthesize** pipeline:

1. **Firecrawl** scrapes/crawls the seed sites into clean Markdown (deep,
   structured extraction the seed URLs you provide).
2. **Tavily** searches the open web for sources the seed set missed (news,
   benchmarks, adjacent vendors) and returns citations.
3. **Context7** resolves authoritative, version-correct docs for any
   library/framework named, so claims are grounded — not hallucinated.
...
Turn a **set of seed URLs into a cited, grounded research brief** in one play
run: the agent deep-scrapes the seeds, fills coverage gaps with web search, and
verifies technical claims against current docs. No single server can do this —
Firecrawl can't ground against versioned docs, Tavily can't deep-crawl a site,
Context7 can't browse the open web.
...
Firecrawl handles *depth* (seed sites → structured content), Tavily handles
*breadth* (open web → fresh sources), Context7 handles *grounding* (claims →
versioned docs). The agent crawls, broadens, grounds, and writes one brief.
...
Activate the play; the engine attaches Firecrawl + Tavily + Context7, and the
agent runs the pipeline. The output is a cited research brief you can paste into
a doc or deck.
...
FrootAI-side cost is model tokens; Firecrawl + Tavily are the third-party
metered calls and scale with crawl depth + search count.
...
All three are `verified-publisher` and expose no destructive tools in this loop.
Keep all three API keys in the environment, never in the play manifest or args.
Only point Firecrawl at sites you are authorized to crawl, and respect each
target's robots/ToS.

## Notes

- **Crawl budget**: cap `firecrawl_crawl` depth + page count so a large site
  doesn't blow the per-run cost; prefer `firecrawl_map` first to scope the crawl.
- **Swap-ins**: replace Tavily with another search source, or add **Notion**
  (see [Recipe 24](./24-research-to-notion.md)) to publish the brief as a page.
