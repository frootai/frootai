...
An agent selects and performs actions in a loop toward a goal, rather than only producing conversational text.

</details>

### 2. When is a multi-agent design unjustified?

<details>
<summary>Expected evidence</summary>

When one bounded agent with tools can complete the task with less coordination, latency, and failure surface.

</details>

### 3. What must terminate every agent loop?

<details>
<summary>Expected evidence</summary>

A success condition, explicit failure/abstention path, cancellation, and a maximum step or cost budget.
