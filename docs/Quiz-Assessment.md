...
> **Assessment facts verified:** 2026-08-03 against the current [Microsoft Foundry architecture](https://learn.microsoft.com/azure/foundry/concepts/architecture), [Foundry model documentation](https://learn.microsoft.com/azure/foundry/foundry-models/concepts/models), and the source modules in this curriculum
...
### Q6: When should you recommend a reasoning model over a general-purpose model?
...
**Why it matters for infra:** Reasoning models can consume additional billed reasoning tokens and have a different latency distribution from general-purpose models. Measure the exact deployed model under representative prompts, then set APIM, retry, and client timeout policies from observed percentiles rather than a generic multiplier.
...
## Section 3: Microsoft Foundry (Module O4)

### Q8: Explain the Foundry-resource-and-project model. Why does it matter for enterprise governance?

<details>
<summary>Click to reveal answer</summary>

**Answer:** Current Microsoft Foundry uses a parent resource with child projects:

- **Foundry resource:** `Microsoft.CognitiveServices/accounts` with `kind: AIServices`. It is the shared boundary for model deployments, agents, evaluations, Foundry Tools, networking, identity, and governance.
...
**Why it matters for infra:** It enables centralized platform governance while preserving project-level application isolation. The topology should follow ownership, data-boundary, networking, quota, and blast-radius requirements rather than a universal one-resource-per-region rule.

**Key insight:** Assign developer access at project scope and reserve Foundry-resource administration for platform owners. Classic Hub/Project deployments use a different Azure ML resource architecture and should follow the official migration guide.

</details>

---

### Q9: What is the difference between managed model APIs and dedicated model compute in Microsoft Foundry?
...
**Why it matters for infra:** Microsoft Foundry provides evaluation workflows and evaluators for these measures. Evaluation remains an engineering concern: version datasets and evaluators, control grader models and costs, retain evidence, and enforce regression thresholds in delivery pipelines.
...
**Key insight:** Semantic Kernel remains relevant for existing kernel and plugin integrations, while Microsoft Agent Framework is its successor for new agent and workflow orchestration. Choose from measured requirements and migration cost, not vendor affiliation alone.
