...
> **Regulatory facts verified:** 2026-08-03 against the European Commission's [AI Act overview](https://digital-strategy.ec.europa.eu/en/policies/regulatory-framework-ai). This module is technical guidance, not legal advice.

| Regulation / Framework | Scope | Key Requirements for Infrastructure |
|---|---|---|
| **EU AI Act** (phased application) | Providers and deployers in scope of Regulation (EU) 2024/1689 | Prohibited-practice rules began in February 2025; GPAI rules in August 2025; transparency rules apply from August 2026; key high-risk obligations begin December 2, 2027 |
...
| **US federal and sector rules** | Agency use plus sector-specific obligations | Verify current OMB, agency, state, and sector requirements; do not treat a superseded executive order as the compliance baseline |
| **Canada** | Federal, provincial, and sector-specific requirements | Verify enacted law and regulator guidance for the deployment date; proposals are not binding law |
...
| **Is my data stored?** | Base-model inferencing is stateless, but optional stateful features store configured data. Content flagged for potential abuse can be stored for human review unless the subscription is approved for modified abuse monitoring. |
| **Who can access my data?** | Other customers and model providers cannot access it. Authorized Microsoft reviewers can access only content selected by the abuse-monitoring process, subject to documented controls. |
| **Can I modify abuse monitoring?** | Eligible managed customers can apply for modified abuse monitoring and verify the resulting `ContentLogging` setting on the resource. |

Validate these controls against the current [Microsoft Foundry data, privacy, and security documentation](https://learn.microsoft.com/azure/ai-foundry/responsible-ai/openai/data-privacy), especially when using preview or stateful features.

### Data Residency

Not all models, deployment types, or platform features are available in all regions. Data residency requirements must inform both region and deployment-type selection.

| Deployment Type | Processing Boundary | Design Implication |
|---|---|---|
| **Global** | Any geography where the selected model is deployed | Do not use when processing must remain in a specific geography or data zone. |
| **DataZone** | Within the selected US or EU data zone | Use when zone-level processing is acceptable; verify that the model and feature support this deployment type. |
| **Standard / Regional** | Within the customer-specified geography, with operational processing possible between regions in that geography | Confirm the exact model, feature, quota, and geography before approval. |

:::tip Check Model Availability
Model availability changes frequently. Check the [Foundry model availability matrix](https://learn.microsoft.com/azure/ai-foundry/openai/concepts/models#model-summary-table-and-region-availability) and [deployment-type guidance](https://learn.microsoft.com/azure/ai-foundry/foundry-models/concepts/deployment-types) before committing to a region or processing boundary.
:::
...
- Select an EU region and a deployment type whose documented processing boundary satisfies the requirement
- Do not assume a resource region constrains processing when using a Global deployment
- Verify where stateful feature data and abuse-monitoring data are stored
...
It addresses only some harmful content, not bias, privacy, grounding, authorization, agency, or process accountability.
...
The tendency for people to over-trust automated recommendations even when evidence is weak or conflicting.

</details>

### 3. When must risk evaluation repeat?

<details>
<summary>Expected evidence</summary>

Whenever models, prompts, data, tools, policies, deployment context, or affected populations materially change.
