...
> **Fine-tuning facts verified:** 2026-08-03 against the [Microsoft Foundry fine-tuning guide](https://learn.microsoft.com/azure/foundry/how-to/fine-tune-models) and [Foundry model catalog](https://learn.microsoft.com/azure/foundry/foundry-models/concepts/models)
...
## T1.9 Microsoft Foundry Fine-Tuning

Microsoft Foundry supports multiple customization methods, but eligibility changes by model version, region, deployment type, and subscription. Treat the portal and current model documentation as the support contract.

| Decision | Verify Before Training |
|----------|------------------------|
| **Model eligibility** | Exact model ID and version support fine-tuning in the target region |
| **Method** | SFT, DPO, RFT, LoRA, or another supported path matches the model and objective |
| **Dataset contract** | Required format, size, validation split, and content policy |
| **Capacity and cost** | Training quota, job pricing, deployment pricing, and regional capacity |
| **Evaluation** | Baseline, holdout set, safety checks, and acceptance thresholds are defined before training |
...
Training cost is a function of method, model, training tokens or compute time, checkpoints, retries, evaluation, and the deployed endpoint. Use the target model's live pricing and quota data; do not estimate a production business case from a curriculum snapshot.
...
|------|---------|-------------|
| **Microsoft Foundry** | Model management, fine-tuning, evaluation | Native Azure |
| **MLflow** | Experiment tracking, model registry | Open source, Azure-integrated |
| **Prompt Flow** | Existing prompt development and evaluation workflows | Verify support in the current Foundry experience and VS Code tooling |
