...
- [Applied Scenario: Operate a Cost-Aware Model Router](#applied-scenario-operate-a-cost-aware-model-router)
...
| **GPU Support** | Serverless A100/T4; quota and region dependent | Node-pool SKU dependent | No native GPU workers | Available when hosted on Container Apps | N/A |
| **Long-running** | Yes, subject to app and ingress limits | Yes | Yes | Plan and trigger dependent | Managed |
| **WebSocket/SSE** | ✅ | ✅ | ✅ | ❌ | ✅ |
...
Recheck the [Container Apps serverless GPU regions and constraints](https://learn.microsoft.com/azure/container-apps/gpu-serverless-overview) and [Functions hosting-plan timeouts](https://learn.microsoft.com/azure/azure-functions/functions-scale#function-app-timeout-duration) during design. Deploying Foundry catalog models directly to Container Apps serverless GPUs is a separate preview capability from running your own GPU container.
...
Different tasks have different quality, latency, contract, safety, and cost requirements.

</details>

### 2. What must happen when the preferred deployment is unavailable?

<details>
<summary>Expected evidence</summary>

A tested fallback, explicit degradation, or fail-closed response must preserve the workload contract.

</details>

### 3. Which telemetry is required to tune a router?

<details>
<summary>Expected evidence</summary>

Per-route quality, latency, errors, fallbacks, token use, cost, and business outcome.
