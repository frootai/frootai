# Recipe 28 Composition Eval: Browser Screenshot to Bug Report (Playwright + Markitdown + GitHub)

> Post-run evaluation check asserting recipe 28 (`28-browser-screenshot-to-bug-report`) still produces valid
...
[`28-browser-screenshot-to-bug-report`](../cookbook/28-browser-screenshot-to-bug-report.md) — harness at
[`cookbook/recipes-mcp-composition/28-browser-screenshot-to-bug-report/run.mjs`](../cookbook/recipes-mcp-composition/28-browser-screenshot-to-bug-report/run.mjs).

## Areas attached

`playwright`, `markitdown`, `github`
...
1. **Runs the recipe harness** — `node cookbook/recipes-mcp-composition/28-browser-screenshot-to-bug-report/run.mjs`.
2. **Asserts the run is valid** — the transcript ends with `RESULT: OK` and the
   process exits 0.
3. **Asserts full federation** — every attached area (`playwright`, `markitdown`, `github`) was exercised;
...
- Every attached area (`playwright`, `markitdown`, `github`) appears in the transcript.
- The committed `sample-output.md` transcript matches the live run.

## Fail Examples

```
FAIL: 28-browser-screenshot-to-bug-report
...
FAIL: 28-browser-screenshot-to-bug-report
  attached area "playwright" was never called during the run
...
```bash
node frootai/cookbook/recipes-mcp-composition/28-browser-screenshot-to-bug-report/run.mjs
```
...
node frootai/cookbook/recipes-mcp-composition/28-browser-screenshot-to-bug-report/run.mjs | tail -1

# Confirm the committed sample matches the live run
diff <(node frootai/cookbook/recipes-mcp-composition/28-browser-screenshot-to-bug-report/run.mjs) \
     <(sed -n '/^```/,/^```/p' frootai/cookbook/recipes-mcp-composition/28-browser-screenshot-to-bug-report/sample-output.md)
```
