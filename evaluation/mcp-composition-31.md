# Recipe 31 Composition Eval: Multi-Cloud Cost Report (Azure + MongoDB)

> Post-run evaluation check asserting recipe 31 (`31-multi-cloud-cost-report`) still produces valid
...
[`31-multi-cloud-cost-report`](../cookbook/31-multi-cloud-cost-report.md) — harness at
[`cookbook/recipes-mcp-composition/31-multi-cloud-cost-report/run.mjs`](../cookbook/recipes-mcp-composition/31-multi-cloud-cost-report/run.mjs).
...
1. **Runs the recipe harness** — `node cookbook/recipes-mcp-composition/31-multi-cloud-cost-report/run.mjs`.
2. **Asserts the run is valid** — the transcript ends with `RESULT: OK` and the
   process exits 0.
3. **Asserts full federation** — every attached area (`azure`, `mongodb`) was exercised;
...
- Every attached area (`azure`, `mongodb`) appears in the transcript.
- The committed `sample-output.md` transcript matches the live run.

## Fail Examples

```
FAIL: 31-multi-cloud-cost-report
...
```bash
node frootai/cookbook/recipes-mcp-composition/31-multi-cloud-cost-report/run.mjs
```
...
node frootai/cookbook/recipes-mcp-composition/31-multi-cloud-cost-report/run.mjs | tail -1

# Confirm the committed sample matches the live run
diff <(node frootai/cookbook/recipes-mcp-composition/31-multi-cloud-cost-report/run.mjs) \
     <(sed -n '/^```/,/^```/p' frootai/cookbook/recipes-mcp-composition/31-multi-cloud-cost-report/sample-output.md)
```
