# Recipe 32 Composition Eval: Firecrawl Research Pipeline (Firecrawl + Tavily + Context7)

> Post-run evaluation check asserting recipe 32 (`32-firecrawl-research-pipeline`) still produces valid
...
[`32-firecrawl-research-pipeline`](../cookbook/32-firecrawl-research-pipeline.md) — harness at
[`cookbook/recipes-mcp-composition/32-firecrawl-research-pipeline/run.mjs`](../cookbook/recipes-mcp-composition/32-firecrawl-research-pipeline/run.mjs).

## Areas attached

`firecrawl`, `tavily-ai`, `context7`
...
1. **Runs the recipe harness** — `node cookbook/recipes-mcp-composition/32-firecrawl-research-pipeline/run.mjs`.
2. **Asserts the run is valid** — the transcript ends with `RESULT: OK` and the
   process exits 0.
3. **Asserts full federation** — every attached area (`firecrawl`, `tavily-ai`, `context7`) was exercised;
...
- Every attached area (`firecrawl`, `tavily-ai`, `context7`) appears in the transcript.
- The committed `sample-output.md` transcript matches the live run.

## Fail Examples

```
FAIL: 32-firecrawl-research-pipeline
...
```bash
node frootai/cookbook/recipes-mcp-composition/32-firecrawl-research-pipeline/run.mjs
```
...
node frootai/cookbook/recipes-mcp-composition/32-firecrawl-research-pipeline/run.mjs | tail -1

# Confirm the committed sample matches the live run
diff <(node frootai/cookbook/recipes-mcp-composition/32-firecrawl-research-pipeline/run.mjs) \
     <(sed -n '/^```/,/^```/p' frootai/cookbook/recipes-mcp-composition/32-firecrawl-research-pipeline/sample-output.md)
```
