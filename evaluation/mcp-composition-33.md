# Recipe 33 Composition Eval: Elastic Log Analysis (Elastic + Context7 + MS Learn)

> Post-run evaluation check asserting recipe 33 (`33-elastic-log-analysis`) still produces valid
...
[`33-elastic-log-analysis`](../cookbook/33-elastic-log-analysis.md) — harness at
[`cookbook/recipes-mcp-composition/33-elastic-log-analysis/run.mjs`](../cookbook/recipes-mcp-composition/33-elastic-log-analysis/run.mjs).

## Areas attached

`elastic`, `context7`, `ms-learn`
...
1. **Runs the recipe harness** — `node cookbook/recipes-mcp-composition/33-elastic-log-analysis/run.mjs`.
2. **Asserts the run is valid** — the transcript ends with `RESULT: OK` and the
   process exits 0.
3. **Asserts full federation** — every attached area (`elastic`, `context7`, `ms-learn`) was exercised;
...
- Every attached area (`elastic`, `context7`, `ms-learn`) appears in the transcript.
- The committed `sample-output.md` transcript matches the live run.

## Fail Examples

```
FAIL: 33-elastic-log-analysis
...
FAIL: 33-elastic-log-analysis
  attached area "elastic" was never called during the run
...
```bash
node frootai/cookbook/recipes-mcp-composition/33-elastic-log-analysis/run.mjs
```
...
node frootai/cookbook/recipes-mcp-composition/33-elastic-log-analysis/run.mjs | tail -1

# Confirm the committed sample matches the live run
diff <(node frootai/cookbook/recipes-mcp-composition/33-elastic-log-analysis/run.mjs) \
     <(sed -n '/^```/,/^```/p' frootai/cookbook/recipes-mcp-composition/33-elastic-log-analysis/sample-output.md)
```
