# Recipe 35 Composition Eval: pgEdge Replication Monitor (pgEdge + Elastic)

> Post-run evaluation check asserting recipe 35 (`35-pgedge-replication-monitor`) still produces valid
...
[`35-pgedge-replication-monitor`](../cookbook/35-pgedge-replication-monitor.md) — harness at
[`cookbook/recipes-mcp-composition/35-pgedge-replication-monitor/run.mjs`](../cookbook/recipes-mcp-composition/35-pgedge-replication-monitor/run.mjs).
...
1. **Runs the recipe harness** — `node cookbook/recipes-mcp-composition/35-pgedge-replication-monitor/run.mjs`.
2. **Asserts the run is valid** — the transcript ends with `RESULT: OK` and the
   process exits 0.
3. **Asserts full federation** — every attached area (`pgedge`, `elastic`) was exercised;
...
- Every attached area (`pgedge`, `elastic`) appears in the transcript.
- The committed `sample-output.md` transcript matches the live run.

## Fail Examples

```
FAIL: 35-pgedge-replication-monitor
...
FAIL: 35-pgedge-replication-monitor
  attached area "pgedge" was never called during the run
...
```bash
node frootai/cookbook/recipes-mcp-composition/35-pgedge-replication-monitor/run.mjs
```
...
node frootai/cookbook/recipes-mcp-composition/35-pgedge-replication-monitor/run.mjs | tail -1

# Confirm the committed sample matches the live run
diff <(node frootai/cookbook/recipes-mcp-composition/35-pgedge-replication-monitor/run.mjs) \
     <(sed -n '/^```/,/^```/p' frootai/cookbook/recipes-mcp-composition/35-pgedge-replication-monitor/sample-output.md)
```
