# Recipe 36 Composition Eval: PDF to Research Deck (Markitdown + Context7 + Tavily)

> Post-run evaluation check asserting recipe 36 (`36-pdf-to-research-deck`) still produces valid
...
[`36-pdf-to-research-deck`](../cookbook/36-pdf-to-research-deck.md) — harness at
[`cookbook/recipes-mcp-composition/36-pdf-to-research-deck/run.mjs`](../cookbook/recipes-mcp-composition/36-pdf-to-research-deck/run.mjs).

## Areas attached

`markitdown`, `context7`, `tavily-ai`
...
1. **Runs the recipe harness** — `node cookbook/recipes-mcp-composition/36-pdf-to-research-deck/run.mjs`.
2. **Asserts the run is valid** — the transcript ends with `RESULT: OK` and the
   process exits 0.
3. **Asserts full federation** — every attached area (`markitdown`, `context7`, `tavily-ai`) was exercised;
...
- Every attached area (`markitdown`, `context7`, `tavily-ai`) appears in the transcript.
- The committed `sample-output.md` transcript matches the live run.

## Fail Examples

```
FAIL: 36-pdf-to-research-deck
...
FAIL: 36-pdf-to-research-deck
  attached area "markitdown" was never called during the run
...
```bash
node frootai/cookbook/recipes-mcp-composition/36-pdf-to-research-deck/run.mjs
```
...
node frootai/cookbook/recipes-mcp-composition/36-pdf-to-research-deck/run.mjs | tail -1

# Confirm the committed sample matches the live run
diff <(node frootai/cookbook/recipes-mcp-composition/36-pdf-to-research-deck/run.mjs) \
     <(sed -n '/^```/,/^```/p' frootai/cookbook/recipes-mcp-composition/36-pdf-to-research-deck/sample-output.md)
```
