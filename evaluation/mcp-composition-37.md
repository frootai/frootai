# Recipe 37 Composition Eval: RAG from a GitHub Repo (GitHub + Markitdown + Azure AI Search)

> Post-run evaluation check asserting recipe 37 (`37-rag-from-github-repo`) still produces valid
...
[`37-rag-from-github-repo`](../cookbook/37-rag-from-github-repo.md) — harness at
[`cookbook/recipes-mcp-composition/37-rag-from-github-repo/run.mjs`](../cookbook/recipes-mcp-composition/37-rag-from-github-repo/run.mjs).

## Areas attached

`github`, `markitdown`, `azure`
...
1. **Runs the recipe harness** — `node cookbook/recipes-mcp-composition/37-rag-from-github-repo/run.mjs`.
2. **Asserts the run is valid** — the transcript ends with `RESULT: OK` and the
   process exits 0.
3. **Asserts full federation** — every attached area (`github`, `markitdown`, `azure`) was exercised;
...
- Every attached area (`github`, `markitdown`, `azure`) appears in the transcript.
- The committed `sample-output.md` transcript matches the live run.

## Fail Examples

```
FAIL: 37-rag-from-github-repo
...
```bash
node frootai/cookbook/recipes-mcp-composition/37-rag-from-github-repo/run.mjs
```
...
node frootai/cookbook/recipes-mcp-composition/37-rag-from-github-repo/run.mjs | tail -1

# Confirm the committed sample matches the live run
diff <(node frootai/cookbook/recipes-mcp-composition/37-rag-from-github-repo/run.mjs) \
     <(sed -n '/^```/,/^```/p' frootai/cookbook/recipes-mcp-composition/37-rag-from-github-repo/sample-output.md)
```
