...
describe('conditional rules', () => {
  ...
  it('rule 4: cultivated with provenance.source=github-api fails', () => {
    const ex = cultivatedExample();
    (ex.provenance as Record<string, unknown>).source = 'github-api';
    const result = validate(ACCELERATOR_SCHEMA, ex);
    expect(result.ok).toBe(false);
    expect(
      result.errors.some((e) => e.keyword === 'const' && e.path.includes('/provenance/source')),
    ).toBe(true);
  });

  it('rule 5: harvested with provenance.source=frootai-greenhouse fails', () => {
    const ex = harvestedExample();
    (ex.provenance as Record<string, unknown>).source = 'frootai-greenhouse';
    const result = validate(ACCELERATOR_SCHEMA, ex);
    expect(result.ok).toBe(false);
    expect(
      result.errors.some((e) => e.keyword === 'const' && e.path.includes('/provenance/source')),
  ...
  it('rule 2: gold_iac=true without gold_iac_reason fails (doctrine line 9 escape valve)', () => {
    ...
    expect(
      result.errors.some(
        (e) =>
          e.message.includes('gold_iac_reason') ||
          e.path.includes('/gold_iac_reason'),
      ),
    ).toBe(true);
  });
  ...
  it('positive: known-good cultivated example passes', () => {
    ...
    expect(
      result.ok,
      `cultivated example should pass; got errors: ${JSON.stringify(result.errors, null, 2)}`,
    ).toBe(true);
  });
});
