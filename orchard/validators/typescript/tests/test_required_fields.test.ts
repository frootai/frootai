...
describe('top-level required fields', () => {
  it.each(REQUIRED_FIELDS)('rejects payload missing %s', (field) => {
    ...
    delete (ex as Record<string, unknown>)[field];
    const result = validate(ACCELERATOR_SCHEMA, ex);

    expect(result.ok).toBe(false);
    const requiredErrors = result.errors.filter((e) => e.keyword === 'required');
    expect(
      requiredErrors.some((e) => e.message.includes(field)),
      `expected a 'required' error mentioning ${field}`,
    ).toBe(true);
  });
});

describe('nested provenance required fields', () => {
  it.each(['harvested_at', 'harvested_by', 'source'])(
    'rejects provenance missing %s',
    (field) => {
      const ex = harvestedExample();
      const provenance = { ...(ex.provenance as Record<string, unknown>) };
