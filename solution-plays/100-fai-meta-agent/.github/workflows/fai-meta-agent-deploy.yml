# AI-Powered Deployment Workflow — Fai Meta Agent

> Layer 3 — Agentic Workflow. Compiles to GitHub Actions for automated Azure deployment.

## Trigger
On push to `main` branch after PR merge, when files in `solution-plays/100-fai-meta-agent/infra/` are modified.
...
```yaml
name: AI Deploy — Fai Meta Agent
on:
  push:
    branches: [main]
    paths: ['solution-plays/100-fai-meta-agent/infra/**']
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Bicep
        run: az bicep build --file solution-plays/100-fai-meta-agent/infra/main.bicep
      - name: Validate Configs
        run: |
          for f in solution-plays/100-fai-meta-agent/config/*.json; do
      ...
      - name: Deploy to Staging
        run: |
          az deployment group create \
            --resource-group rg-fai-meta-agent-staging \
            --template-file solution-plays/100-fai-meta-agent/infra/main.bicep \
            --parameters solution-plays/100-fai-meta-agent/infra/parameters.json
  ...
  evaluate:
    ...
    steps:
      ...
      - name: Run Evaluation
        run: python solution-plays/100-fai-meta-agent/evaluation/eval.py
        env:
  ...
  deploy-production:
    ...
    steps:
      ...
      - name: Deploy to Production
        run: |
          az deployment group create \
            --resource-group rg-fai-meta-agent \
            --template-file solution-plays/100-fai-meta-agent/infra/main.bicep \
            --parameters solution-plays/100-fai-meta-agent/infra/parameters.json \
