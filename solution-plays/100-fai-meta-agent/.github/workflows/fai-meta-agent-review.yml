# AI-Powered Code Review Workflow — Fai Meta Agent

> Layer 3 — Agentic Workflow. Runs automated review on PRs targeting this play.

## Trigger
On pull request to `main` when files in `solution-plays/100-fai-meta-agent/` are modified.
...
```yaml
name: AI Review — Fai Meta Agent
on:
  pull_request:
    paths: ['solution-plays/100-fai-meta-agent/**']
  ...
  review:
    ...
    steps:
      ...
      - name: Check JSON validity
        run: |
          for f in $(find solution-plays/100-fai-meta-agent -name "*.json" -type f); do
            python3 -c "import json; json.load(open('$f'))" || echo "INVALID: $f"
          done

      - name: Check for secrets
        run: |
          if grep -rl "sk-[a-zA-Z0-9]\{20,\}" solution-plays/100-fai-meta-agent/ 2>/dev/null; then
      ...
      - name: Check guardrails
        run: |
          python3 -c "
          import json
          g = json.load(open('solution-plays/100-fai-meta-agent/config/guardrails.json'))
