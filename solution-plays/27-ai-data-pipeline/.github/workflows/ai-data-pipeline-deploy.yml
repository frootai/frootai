# AI-Powered Deployment Workflow — Ai Data Pipeline

> Layer 3 — Agentic Workflow. Compiles to GitHub Actions for automated Azure deployment.

## Trigger
On push to `main` branch after PR merge, when files in `solution-plays/27-ai-data-pipeline/infra/` are modified.
...
```yaml
name: AI Deploy — Ai Data Pipeline
on:
  push:
    branches: [main]
    paths: ['solution-plays/27-ai-data-pipeline/infra/**']
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Bicep
        run: az bicep build --file solution-plays/27-ai-data-pipeline/infra/main.bicep
      - name: Validate Configs
        run: |
          for f in solution-plays/27-ai-data-pipeline/config/*.json; do
      ...
      - name: Deploy to Staging
        run: |
          az deployment group create \
            --resource-group rg-ai-data-pipeline-staging \
            --template-file solution-plays/27-ai-data-pipeline/infra/main.bicep \
            --parameters solution-plays/27-ai-data-pipeline/infra/parameters.json
  ...
  evaluate:
    ...
    steps:
      ...
      - name: Run Evaluation
        run: python solution-plays/27-ai-data-pipeline/evaluation/eval.py
        env:
  ...
  deploy-production:
    ...
    steps:
      ...
      - name: Deploy to Production
        run: |
          az deployment group create \
            --resource-group rg-ai-data-pipeline \
            --template-file solution-plays/27-ai-data-pipeline/infra/main.bicep \
            --parameters solution-plays/27-ai-data-pipeline/infra/parameters.json \
