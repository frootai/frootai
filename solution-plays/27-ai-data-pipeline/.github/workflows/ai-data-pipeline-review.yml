# AI-Powered Code Review Workflow — Ai Data Pipeline

> Layer 3 — Agentic Workflow. Runs automated review on PRs targeting this play.

## Trigger
On pull request to `main` when files in `solution-plays/27-ai-data-pipeline/` are modified.
...
```yaml
name: AI Review — Ai Data Pipeline
on:
  pull_request:
    paths: ['solution-plays/27-ai-data-pipeline/**']
  ...
  review:
    ...
    steps:
      ...
      - name: Check JSON validity
        run: |
          for f in $(find solution-plays/27-ai-data-pipeline -name "*.json" -type f); do
            python3 -c "import json; json.load(open('$f'))" || echo "INVALID: $f"
          done

      - name: Check for secrets
        run: |
          if grep -rl "sk-[a-zA-Z0-9]\{20,\}" solution-plays/27-ai-data-pipeline/ 2>/dev/null; then
      ...
      - name: Check guardrails
        run: |
          python3 -c "
          import json
          g = json.load(open('solution-plays/27-ai-data-pipeline/config/guardrails.json'))
