# AI-Powered Deployment Workflow — Mcp Gateway

> Layer 3 — Agentic Workflow. Compiles to GitHub Actions for automated Azure deployment.

## Trigger
On push to `main` branch after PR merge, when files in `solution-plays/29-mcp-gateway/infra/` are modified.
...
```yaml
name: AI Deploy — Mcp Gateway
on:
  push:
    branches: [main]
    paths: ['solution-plays/29-mcp-gateway/infra/**']
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Bicep
        run: az bicep build --file solution-plays/29-mcp-gateway/infra/main.bicep
      - name: Validate Configs
        run: |
          for f in solution-plays/29-mcp-gateway/config/*.json; do
      ...
      - name: Deploy to Staging
        run: |
          az deployment group create \
            --resource-group rg-mcp-gateway-staging \
            --template-file solution-plays/29-mcp-gateway/infra/main.bicep \
            --parameters solution-plays/29-mcp-gateway/infra/parameters.json
  ...
  evaluate:
    ...
    steps:
      ...
      - name: Run Evaluation
        run: python solution-plays/29-mcp-gateway/evaluation/eval.py
        env:
