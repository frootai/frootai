# AI-Powered Code Review Workflow — Mcp Gateway

> Layer 3 — Agentic Workflow. Runs automated review on PRs targeting this play.

## Trigger
On pull request to `main` when files in `solution-plays/29-mcp-gateway/` are modified.
...
```yaml
name: AI Review — Mcp Gateway
on:
  pull_request:
    paths: ['solution-plays/29-mcp-gateway/**']
  ...
  review:
    ...
    steps:
      ...
      - name: Check JSON validity
        run: |
          for f in $(find solution-plays/29-mcp-gateway -name "*.json" -type f); do
            python3 -c "import json; json.load(open('$f'))" || echo "INVALID: $f"
          done

      - name: Check for secrets
        run: |
          if grep -rl "sk-[a-zA-Z0-9]\{20,\}" solution-plays/29-mcp-gateway/ 2>/dev/null; then
      ...
      - name: Check guardrails
        run: |
          python3 -c "
          import json
          g = json.load(open('solution-plays/29-mcp-gateway/config/guardrails.json'))
