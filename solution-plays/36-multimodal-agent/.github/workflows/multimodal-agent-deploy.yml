# AI-Powered Deployment Workflow — Multimodal Agent

> Layer 3 — Agentic Workflow. Compiles to GitHub Actions for automated Azure deployment.

## Trigger
On push to `main` branch after PR merge, when files in `solution-plays/36-multimodal-agent/infra/` are modified.
...
```yaml
name: AI Deploy — Multimodal Agent
on:
  push:
    branches: [main]
    paths: ['solution-plays/36-multimodal-agent/infra/**']
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Configs
        run: |
          for f in solution-plays/36-multimodal-agent/config/*.json; do
      ...
      - name: Deploy to Staging
        run: |
          az deployment group create \
            --resource-group rg-multimodal-agent-staging \
            --template-file solution-plays/36-multimodal-agent/infra/main.bicep \
            --parameters solution-plays/36-multimodal-agent/infra/parameters.json
  ...
  evaluate:
    ...
    steps:
      ...
      - name: Run Evaluation
        run: python solution-plays/36-multimodal-agent/evaluation/eval.py
        env:
  ...
  deploy-production:
    ...
    steps:
      ...
      - name: Deploy to Production
        run: |
          az deployment group create \
            --resource-group rg-multimodal-agent \
            --template-file solution-plays/36-multimodal-agent/infra/main.bicep \
            --parameters solution-plays/36-multimodal-agent/infra/parameters.json \
