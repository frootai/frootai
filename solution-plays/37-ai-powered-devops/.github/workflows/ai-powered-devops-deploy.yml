# AI-Powered Deployment Workflow — Ai Powered Devops

> Layer 3 — Agentic Workflow. Compiles to GitHub Actions for automated Azure deployment.

## Trigger
On push to `main` branch after PR merge, when files in `solution-plays/37-ai-powered-devops/infra/` are modified.
...
```yaml
name: AI Deploy — Ai Powered Devops
on:
  push:
    branches: [main]
    paths: ['solution-plays/37-ai-powered-devops/infra/**']
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Bicep
        run: az bicep build --file solution-plays/37-ai-powered-devops/infra/main.bicep
      - name: Validate Configs
        run: |
          for f in solution-plays/37-ai-powered-devops/config/*.json; do
      ...
      - name: Deploy to Staging
        run: |
          az deployment group create \
            --resource-group rg-ai-powered-devops-staging \
            --template-file solution-plays/37-ai-powered-devops/infra/main.bicep \
            --parameters solution-plays/37-ai-powered-devops/infra/parameters.json
  ...
  evaluate:
    ...
    steps:
      ...
      - name: Run Evaluation
        run: python solution-plays/37-ai-powered-devops/evaluation/eval.py
        env:
  ...
  deploy-production:
    ...
    steps:
      ...
      - name: Deploy to Production
        run: |
          az deployment group create \
            --resource-group rg-ai-powered-devops \
            --template-file solution-plays/37-ai-powered-devops/infra/main.bicep \
            --parameters solution-plays/37-ai-powered-devops/infra/parameters.json \
