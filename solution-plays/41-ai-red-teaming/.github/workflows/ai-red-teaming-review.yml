# AI-Powered Code Review Workflow — Ai Red Teaming

> Layer 3 — Agentic Workflow. Runs automated review on PRs targeting this play.

## Trigger
On pull request to `main` when files in `solution-plays/41-ai-red-teaming/` are modified.
...
```yaml
name: AI Review — Ai Red Teaming
on:
  pull_request:
    paths: ['solution-plays/41-ai-red-teaming/**']
  ...
  review:
    ...
    steps:
      ...
      - name: Check JSON validity
        run: |
          for f in $(find solution-plays/41-ai-red-teaming -name "*.json" -type f); do
            python3 -c "import json; json.load(open('$f'))" || echo "INVALID: $f"
          done

      - name: Check for secrets
        run: |
          if grep -rl "sk-[a-zA-Z0-9]\{20,\}" solution-plays/41-ai-red-teaming/ 2>/dev/null; then
      ...
      - name: Check guardrails
        run: |
          python3 -c "
          import json
          g = json.load(open('solution-plays/41-ai-red-teaming/config/guardrails.json'))
