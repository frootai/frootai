# AI-Powered Code Review Workflow — Foundry Local On Device

> Layer 3 — Agentic Workflow. Runs automated review on PRs targeting this play.

## Trigger
On pull request to `main` when files in `solution-plays/44-foundry-local-on-device/` are modified.
...
```yaml
name: AI Review — Foundry Local On Device
on:
  pull_request:
    paths: ['solution-plays/44-foundry-local-on-device/**']
  ...
  review:
    ...
    steps:
      ...
      - name: Check JSON validity
        run: |
          for f in $(find solution-plays/44-foundry-local-on-device -name "*.json" -type f); do
            python3 -c "import json; json.load(open('$f'))" || echo "INVALID: $f"
          done

      - name: Check for secrets
        run: |
          if grep -rl "sk-[a-zA-Z0-9]\{20,\}" solution-plays/44-foundry-local-on-device/ 2>/dev/null; then
      ...
      - name: Check guardrails
        run: |
          python3 -c "
          import json
          g = json.load(open('solution-plays/44-foundry-local-on-device/config/guardrails.json'))
