# AI-Powered Deployment Workflow — Realtime Event Ai

> Layer 3 — Agentic Workflow. Compiles to GitHub Actions for automated Azure deployment.

## Trigger
On push to `main` branch after PR merge, when files in `solution-plays/45-realtime-event-ai/infra/` are modified.
...
```yaml
name: AI Deploy — Realtime Event Ai
on:
  push:
    branches: [main]
    paths: ['solution-plays/45-realtime-event-ai/infra/**']
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Configs
        run: |
          for f in solution-plays/45-realtime-event-ai/config/*.json; do
      ...
      - name: Deploy to Staging
        run: |
          az deployment group create \
            --resource-group rg-realtime-event-ai-staging \
            --template-file solution-plays/45-realtime-event-ai/infra/main.bicep \
            --parameters solution-plays/45-realtime-event-ai/infra/parameters.json
  ...
  evaluate:
    ...
    steps:
      ...
      - name: Run Evaluation
        run: python solution-plays/45-realtime-event-ai/evaluation/eval.py
        env:
  ...
  deploy-production:
    ...
    steps:
      ...
      - name: Deploy to Production
        run: |
          az deployment group create \
            --resource-group rg-realtime-event-ai \
            --template-file solution-plays/45-realtime-event-ai/infra/main.bicep \
            --parameters solution-plays/45-realtime-event-ai/infra/parameters.json \
