# AI-Powered Deployment Workflow — Ai Api Gateway V2

> Layer 3 — Agentic Workflow. Compiles to GitHub Actions for automated Azure deployment.

## Trigger
On push to `main` branch after PR merge, when files in `solution-plays/52-ai-api-gateway-v2/infra/` are modified.
...
```yaml
name: AI Deploy — Ai Api Gateway V2
on:
  push:
    branches: [main]
    paths: ['solution-plays/52-ai-api-gateway-v2/infra/**']
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Bicep
        run: az bicep build --file solution-plays/52-ai-api-gateway-v2/infra/main.bicep
      - name: Validate Configs
        run: |
          for f in solution-plays/52-ai-api-gateway-v2/config/*.json; do
      ...
      - name: Deploy to Staging
        run: |
          az deployment group create \
            --resource-group rg-ai-api-gateway-v2-staging \
            --template-file solution-plays/52-ai-api-gateway-v2/infra/main.bicep \
            --parameters solution-plays/52-ai-api-gateway-v2/infra/parameters.json
  ...
  evaluate:
    ...
    steps:
      ...
      - name: Run Evaluation
        run: python solution-plays/52-ai-api-gateway-v2/evaluation/eval.py
        env:
  ...
  deploy-production:
    ...
    steps:
      ...
      - name: Deploy to Production
        run: |
          az deployment group create \
            --resource-group rg-ai-api-gateway-v2 \
            --template-file solution-plays/52-ai-api-gateway-v2/infra/main.bicep \
            --parameters solution-plays/52-ai-api-gateway-v2/infra/parameters.json \
