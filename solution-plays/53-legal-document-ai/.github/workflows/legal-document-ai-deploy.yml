# AI-Powered Deployment Workflow — Legal Document Ai

> Layer 3 — Agentic Workflow. Compiles to GitHub Actions for automated Azure deployment.

## Trigger
On push to `main` branch after PR merge, when files in `solution-plays/53-legal-document-ai/infra/` are modified.
...
```yaml
name: AI Deploy — Legal Document Ai
on:
  push:
    branches: [main]
    paths: ['solution-plays/53-legal-document-ai/infra/**']
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Bicep
        run: az bicep build --file solution-plays/53-legal-document-ai/infra/main.bicep
      - name: Validate Configs
        run: |
          for f in solution-plays/53-legal-document-ai/config/*.json; do
      ...
      - name: Deploy to Staging
        run: |
          az deployment group create \
            --resource-group rg-legal-document-ai-staging \
            --template-file solution-plays/53-legal-document-ai/infra/main.bicep \
            --parameters solution-plays/53-legal-document-ai/infra/parameters.json
  ...
  evaluate:
    ...
    steps:
      ...
      - name: Run Evaluation
        run: python solution-plays/53-legal-document-ai/evaluation/eval.py
        env:
  ...
  deploy-production:
    ...
    steps:
      ...
      - name: Deploy to Production
        run: |
          az deployment group create \
            --resource-group rg-legal-document-ai \
            --template-file solution-plays/53-legal-document-ai/infra/main.bicep \
            --parameters solution-plays/53-legal-document-ai/infra/parameters.json \
