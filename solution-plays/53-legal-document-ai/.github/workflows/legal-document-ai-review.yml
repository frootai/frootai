# AI-Powered Code Review Workflow — Legal Document Ai

> Layer 3 — Agentic Workflow. Runs automated review on PRs targeting this play.

## Trigger
On pull request to `main` when files in `solution-plays/53-legal-document-ai/` are modified.
...
```yaml
name: AI Review — Legal Document Ai
on:
  pull_request:
    paths: ['solution-plays/53-legal-document-ai/**']
  ...
  review:
    ...
    steps:
      ...
      - name: Check JSON validity
        run: |
          for f in $(find solution-plays/53-legal-document-ai -name "*.json" -type f); do
            python3 -c "import json; json.load(open('$f'))" || echo "INVALID: $f"
          done

      - name: Check for secrets
        run: |
          if grep -rl "sk-[a-zA-Z0-9]\{20,\}" solution-plays/53-legal-document-ai/ 2>/dev/null; then
      ...
      - name: Check guardrails
        run: |
          python3 -c "
          import json
          g = json.load(open('solution-plays/53-legal-document-ai/config/guardrails.json'))
