# AI-Powered Deployment Workflow — Supply Chain Ai

> Layer 3 — Agentic Workflow. Compiles to GitHub Actions for automated Azure deployment.

## Trigger
On push to `main` branch after PR merge, when files in `solution-plays/55-supply-chain-ai/infra/` are modified.
...
```yaml
name: AI Deploy — Supply Chain Ai
on:
  push:
    branches: [main]
    paths: ['solution-plays/55-supply-chain-ai/infra/**']
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Bicep
        run: az bicep build --file solution-plays/55-supply-chain-ai/infra/main.bicep
      - name: Validate Configs
        run: |
          for f in solution-plays/55-supply-chain-ai/config/*.json; do
      ...
      - name: Deploy to Staging
        run: |
          az deployment group create \
            --resource-group rg-supply-chain-ai-staging \
            --template-file solution-plays/55-supply-chain-ai/infra/main.bicep \
            --parameters solution-plays/55-supply-chain-ai/infra/parameters.json
  ...
  evaluate:
    ...
    steps:
      ...
      - name: Run Evaluation
        run: python solution-plays/55-supply-chain-ai/evaluation/eval.py
        env:
  ...
  deploy-production:
    ...
    steps:
      ...
      - name: Deploy to Production
        run: |
          az deployment group create \
            --resource-group rg-supply-chain-ai \
            --template-file solution-plays/55-supply-chain-ai/infra/main.bicep \
            --parameters solution-plays/55-supply-chain-ai/infra/parameters.json \
