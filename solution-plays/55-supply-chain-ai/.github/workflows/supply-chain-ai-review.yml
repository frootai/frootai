# AI-Powered Code Review Workflow — Supply Chain Ai

> Layer 3 — Agentic Workflow. Runs automated review on PRs targeting this play.

## Trigger
On pull request to `main` when files in `solution-plays/55-supply-chain-ai/` are modified.
...
```yaml
name: AI Review — Supply Chain Ai
on:
  pull_request:
    paths: ['solution-plays/55-supply-chain-ai/**']
  ...
  review:
    ...
    steps:
      ...
      - name: Check JSON validity
        run: |
          for f in $(find solution-plays/55-supply-chain-ai -name "*.json" -type f); do
            python3 -c "import json; json.load(open('$f'))" || echo "INVALID: $f"
          done

      - name: Check for secrets
        run: |
          if grep -rl "sk-[a-zA-Z0-9]\{20,\}" solution-plays/55-supply-chain-ai/ 2>/dev/null; then
      ...
      - name: Check guardrails
        run: |
          python3 -c "
          import json
          g = json.load(open('solution-plays/55-supply-chain-ai/config/guardrails.json'))
