...
class McpToolProvider {
  ...
  getChildren(element) {
    ...
    const items = [
      ...
      { label: "npm — frootai-mcp", desc: "Node.js MCP runtime", icon: "package", color: "charts.orange", url: "https://www.npmjs.com/package/frootai-mcp" },
      { label: "PyPI — frootai-mcp", desc: "Python MCP runtime", icon: "package", color: "charts.blue", url: "https://pypi.org/project/frootai-mcp/" },
...
class DevHubProvider {
  ...
  getChildren(element) {
    ...
    const items = [
      ...
      { label: "API Reference", desc: "Runtime-discovered MCP tools and commands", icon: "radio-tower", color: "charts.purple", url: "https://frootai.dev/docs/api-reference" },
...
class WelcomeTreeProvider {
  ...
  getChildren() {
    ...
    const account = new vscode.TreeItem("Account", vscode.TreeItemCollapsibleState.None);
    account.description = "Sign in and connect Agent FAI";
    account.iconPath = new vscode.ThemeIcon("account", new vscode.ThemeColor("charts.green"));
    account.command = { command: "frootai.account.open", title: "FrootAI Account" };
    ...
    const intelligence = new vscode.TreeItem("Repository Intelligence", vscode.TreeItemCollapsibleState.None);
    intelligence.description = "Map this codebase to Solution Plays";
    intelligence.iconPath = new vscode.ThemeIcon("graph", new vscode.ThemeColor("charts.blue"));
    intelligence.command = { command: "frootai.repository.analyze", title: "Repository Intelligence" };

    return [hi, account, agent, intelligence, setup, search];
  }
}
...
docker run -i ghcr.io/frootai/frootai-mcp  # Docker</pre>
        ...
        <table>
          ...
          <tr><td>🔧 FAI MCP Tools</td><td>Runtime-discovered knowledge, live, chain, ecosystem, and engine tools</td></tr>
        </table>
        ...
        <div class="section">
          <p><strong>VS Code Extension</strong> — native FrootAI product workbench</p>
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("frootai.initDevKit", async (preSelectedPlay) => {
      return vscode.commands.executeCommand("frootai.secureInitPlayKit", "devkit", preSelectedPlay);
      /* Legacy implementation retained below only for source history; unreachable by design. */
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("frootai.initTuneKit", async (preSelectedPlay) => {
      return vscode.commands.executeCommand("frootai.secureInitPlayKit", "tunekit", preSelectedPlay);
      /* Legacy implementation retained below only for source history; unreachable by design. */
      const wsFolder = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
      if (!wsFolder) { vscode.window.showWarningMessage("Open a folder first."); return; }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("frootai.initSpecKit", async (preSelectedPlay) => {
      return vscode.commands.executeCommand("frootai.secureInitPlayKit", "speckit", preSelectedPlay);
      /* Legacy implementation retained below only for source history; unreachable by design. */
      const wsFolder = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
      if (!wsFolder) { vscode.window.showWarningMessage("Open a folder first."); return; }
  ...
  // Accepts optional preSelectedPlay arg (same pattern as initDevKit/initHooks/initPrompts).
  // When called from PlayDetail webview, skips the picker and installs THIS play immediately.
  context.subscriptions.push(
    vscode.commands.registerCommand("frootai.installPlugin", async (preSelectedPlay) => {
      let pickValue;
      if (preSelectedPlay && preSelectedPlay.dir) {
        pickValue = preSelectedPlay.dir;
      } else {
        const plugins = [
      ...
      // Agent files
      const agentContent = `---\ndescription: "Builder agent for ${pickValue}"\ntools:\n  - frootai\n---\n# Builder Agent\nUse FrootAI MCP for architecture patterns and best practices for ${pickValue}.\n`;
      ...
      // SpecKit
      fs.writeFileSync(path.join(wsRoot, "spec/play-spec.json"), JSON.stringify({ name: pickValue, version: "0.1.0", waf_alignment: { reliability: "unchecked", security: "unchecked", cost_optimization: "unchecked", operational_excellence: "unchecked", performance_efficiency: "unchecked", responsible_ai: "unchecked" } }, null, 2));

      // plugin.json
      fs.writeFileSync(path.join(wsRoot, "plugin.json"), JSON.stringify({ name: pickValue, version: "1.0.0", layers: { instructions: [".github/instructions/*.md"], agents: [".github/agents/*.md"], config: ["config/*.json"], spec: ["spec/*.json"] }, dependencies: ["frootai-mcp"], install: "copy" }, null, 2));

      vscode.window.showInformationMessage(`✅ Plugin "${pickValue}" installed! Files: .github/agents, config, spec, plugin.json`);
    })
  );

  // ── Estimate Cost for a specific play (used by PlayDetail webview) ──
  // Same pattern as initDevKit: accepts optional preSelectedPlay; loads cost.json from local repo
  // → GitHub fallback → built-in fallback. Renders a webview panel.
  context.subscriptions.push(
    vscode.commands.registerCommand("frootai.estimateCostForPlay", async (preSelectedPlay) => {
      let selectedPlay = preSelectedPlay;
      if (!selectedPlay) {
        const plays = SOLUTION_PLAYS.map(p => ({ label: `${p.icon} ${p.id} — ${p.name}`, description: p.status, value: p }));
        const pick = await vscode.window.showQuickPick(plays, { placeHolder: "Estimate cost for which solution play?" });
        if (!pick) return;
        selectedPlay = pick.value;
      }

      let costData = null;
      // Try 1: Local workspace repo
      if (root) {
        const localPath = path.join(root, "solution-plays", selectedPlay.dir, "cost.json");
        try { if (fs.existsSync(localPath)) { costData = JSON.parse(fs.readFileSync(localPath, "utf-8")); } } catch (_e) { /* ignore */ }
      }
      // Try 2: GitHub
      if (!costData) {
        try {
          const text = await downloadFromGitHub(`solution-plays/${selectedPlay.dir}/cost.json`);
          costData = JSON.parse(text);
        } catch (_e) { /* ignore */ }
      }

      const totals = costData?.totals || { dev: 150, prod: 700, ent: 2500 };
      const services = costData?.services || [];
      const tips = costData?.optimization_tips || [];

      const catColors = /** @type {Record<string, string>} */ ({ "AI Services": "#10b981", "Compute": "#3b82f6", "Storage": "#f59e0b", "Security": "#7c3aed", "Monitoring": "#0ea5e9" });
      ...
      let servicesHtml = "";
      const maxProd = Math.max(...services.map(/** @type {(s: any) => number} */(s) => s.tiers?.prod?.cost || 0), 1);
      for (const [catName, svcs] of Object.entries(groups)) {
        const color = catColors[catName] || "#6b7280";
        const catTotal = svcs.reduce(/** @type {(a: number, s: any) => number} */(a, s) => a + (s.tiers?.prod?.cost || 0), 0);
        const rows = svcs.map(/** @type {(s: any) => string} */(s) => {
          ...
          return `<tr><td style="padding:8px 12px"><strong>${s.name}</strong><div style="font-size:10px;opacity:0.5;margin-top:2px">${s.purpose || ""}</div></td>
            <td style="padding:8px 12px;text-align:right">$${d}</td>
            <td style="padding:8px 12px;text-align:right">$${p}</td>
            <td style="padding:8px 12px;text-align:right">$${e}</td>
            <td style="padding:8px 12px;width:100px"><div style="height:6px;border-radius:3px;background:#333"><div style="height:100%;width:${barW}%;background:${color};border-radius:3px"></div></div></td></tr>`;
        }).join("");
        servicesHtml += `<div style="margin-bottom:20px">
          ...
          <table style="width:100%;border-collapse:collapse"><tr><th style="text-align:left;padding:6px 12px;border-bottom:1px solid #444;font-size:11px;text-transform:uppercase;opacity:0.5">Service</th><th style="text-align:right;padding:6px 12px;font-size:11px;text-transform:uppercase;opacity:0.5">Dev</th><th style="text-align:right;padding:6px 12px;font-size:11px;text-transform:uppercase;opacity:0.5">Prod</th><th style="text-align:right;padding:6px 12px;font-size:11px;text-transform:uppercase;opacity:0.5">Enterprise</th><th></th></tr>${rows}</table>
        </div>`;
      }
      if (!services.length) {
        servicesHtml = `<p>Detailed cost.json not found for this play. Showing fallback estimates.</p><p style="opacity:0.6">Services: ${selectedPlay.infra || "Azure OpenAI · Container Apps"}</p>`;
      }
      const tipsHtml = tips.map(/** @type {(t: string) => string} */(t) => `<div style="padding:8px 0;font-size:12px;border-bottom:1px solid #333">• ${t}</div>`).join("");

      const panel = vscode.window.createWebviewPanel("frootai.costEstimateForPlay", `Cost — ${selectedPlay.name}`, vscode.ViewColumn.One, { enableScripts: false });
      panel.webview.html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
body{background:#1e1e1e;color:#ccc;font-family:system-ui;padding:24px;line-height:1.6}
...
<h1>Cost Estimate — ${selectedPlay.name}</h1>
<p style="opacity:0.7;font-size:13px">${selectedPlay.desc || ""}</p>
<div class="total-bar">
...
<p class="note">Source: solution-plays/${selectedPlay.dir}/cost.json · <a href="https://azure.microsoft.com/pricing/calculator/" style="color:#3b82f6">Open Azure Pricing Calculator ↗</a></p>
</body></html>`;
    })
  );

  // ── Architecture Diagram for a specific play (used by PlayDetail webview) ──
  // Same pattern: optional preSelectedPlay; loads architecture.md from local repo → GitHub.
  context.subscriptions.push(
    vscode.commands.registerCommand("frootai.showArchitectureDiagram", async (preSelectedPlay) => {
      let selectedPlay = preSelectedPlay;
      if (!selectedPlay) {
        const plays = SOLUTION_PLAYS.map(p => ({ label: `${p.icon} ${p.id} — ${p.name}`, description: p.status, value: p }));
        const pick = await vscode.window.showQuickPick(plays, { placeHolder: "Show architecture diagram for which play?" });
        if (!pick) return;
        selectedPlay = pick.value;
      }
      ...
      let archContent = "";
      if (root) {
        const localPath = path.join(root, "solution-plays", selectedPlay.dir, "architecture.md");
        try { if (fs.existsSync(localPath)) { archContent = fs.readFileSync(localPath, "utf-8"); } } catch (_e) { /* ignore */ }
      }
      if (!archContent) {
        try { archContent = await downloadFromGitHub(`solution-plays/${selectedPlay.dir}/architecture.md`); } catch (_e) { /* ignore */ }
      }

      // enableScripts:true required because markdownToHtml uses marked.js + mermaid via inline <script>
      const panel = vscode.window.createWebviewPanel("frootai.architectureDiagram", `Architecture — ${selectedPlay.name}`, vscode.ViewColumn.One, { enableScripts: true });

      if (archContent) {
        // Append a footer with source link to the markdown so it gets rendered too
        const withFooter = `${archContent}\n\n---\n\n*Source: \`solution-plays/${selectedPlay.dir}/architecture.md\` · [View on GitHub](https://github.com/frootai/frootai/blob/main/solution-plays/${selectedPlay.dir}/architecture.md) ↗*`;
        panel.webview.html = markdownToHtml(withFooter, `Architecture — ${selectedPlay.name}`);
      } else {
        // Fallback HTML when architecture.md isn't available — no scripts needed for this branch
        const svcs = (selectedPlay.infra || "Azure OpenAI · Container Apps").split("·").map(/** @type {(s: string) => string} */(s) => s.trim());
        const fallbackBody = `<h1>Architecture — ${selectedPlay.name}</h1>
          <p>Detailed architecture.md not found for this play.</p>
          <h2>Services (${svcs.length})</h2>
          <ul>${svcs.map(/** @type {(s: string) => string} */(s) => `<li>${s}</li>`).join("")}</ul>
          <p style="opacity:0.6">Add <code>architecture.md</code> to <code>solution-plays/${selectedPlay.dir}/</code> for detailed documentation.</p>`;
        panel.webview.html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
...
li{margin:4px 0;font-size:13px}ul,ol{padding-left:20px;margin:8px 0}
.note{font-size:11px;opacity:0.5;margin-top:24px;border-top:1px solid #333;padding-top:12px}
</style></head><body>
${fallbackBody}
<p class="note">Source: solution-plays/${selectedPlay.dir}/architecture.md · <a href="https://github.com/frootai/frootai/blob/main/solution-plays/${selectedPlay.dir}/architecture.md" style="color:#3b82f6">View on GitHub ↗</a></p>
</body></html>`;
      }
    })
  );
  ...
  // ── Run Evaluation ──
  context.subscriptions.push(
    vscode.commands.registerCommand("frootai.runEvaluation", async (context) => {
      const folders = vscode.workspace.workspaceFolders;
      if (!folders) { vscode.window.showWarningMessage("Open a workspace first."); return; }
      const requestedRoot = context && typeof context.fsPath === "string" ? path.resolve(context.fsPath) : null;
      const selectedFolder = requestedRoot ? folders.find(folder => path.resolve(folder.uri.fsPath) === requestedRoot) : folders.length === 1 ? folders[0] : await vscode.window.showQuickPick(folders.map(folder => ({ label: folder.name, description: folder.uri.fsPath, folder })), { placeHolder: context?.playId ? `Choose the workspace containing Play ${context.playId} evaluation assets` : "Choose the workspace to evaluate" }).then(item => item?.folder);
      if (!selectedFolder) return;
      const wsRoot = selectedFolder.uri.fsPath;
  ...
  statusBar.tooltip = `FrootAI — AI Architecture Knowledge Engine\n${knowledgeLoaded ? `${Object.keys(KNOWLEDGE.modules).length} modules · ${Object.keys(GLOSSARY).length} terms · runtime-discovered MCP tools` : "Knowledge loading..."}`;
