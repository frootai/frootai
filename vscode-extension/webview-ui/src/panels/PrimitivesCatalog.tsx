...
export type CategoryId = "agents" | "skills" | "instructions" | "hooks" | "plugins";
...
// ─── Component ───
export default function PrimitivesCatalog({ primitives, initialCategory, initialId }: { primitives: Record<CategoryId, PrimitiveItem[]>; initialCategory?: string; initialId?: string }) {
  const normalizedInitialCategory = isCategory(initialCategory) ? initialCategory : "agents";
  const [activeTab, setActiveTab] = useState<CategoryId>(normalizedInitialCategory);
  ...
  useEffect(() => {
    if (!isCategory(initialCategory)) return;
    setActiveTab(initialCategory);
    setSearch(""); setSubCat("all"); setWafFilter(null); setPage(1);
    setSelectedItem(initialId ? (primitives[initialCategory] ?? []).find((item) => item.id === decodeURIComponent(initialId)) ?? null : null);
  }, [initialCategory, initialId, primitives]);
...
function PrimitiveCard({ item, category, meta, onClick, onGithub, onInstall, installLabel }: {
  ...
  return (
    <article
      className="card"
      style={{ padding: "12px 14px", borderRadius: 8, transition: "border-color 0.2s", borderColor: "var(--vscode-panel-border)" }}
      onMouseEnter={(e) => (e.currentTarget.style.borderColor = meta.color)}
      onMouseLeave={(e) => (e.currentTarget.style.borderColor = "var(--vscode-panel-border)")}
    >
      <button type="button" onClick={onClick} aria-label={`Open ${displayName} details`} style={{ display: "block", width: "100%", padding: 0, color: "inherit", textAlign: "left", background: "transparent", border: 0, cursor: "pointer" }}>
...
function isCategory(value?: string): value is CategoryId { return value === "agents" || value === "skills" || value === "instructions" || value === "hooks" || value === "plugins"; }
