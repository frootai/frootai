...
const ENGINE_MODULES = [
  ...
  { id: "mcp-bridge", Icon: Cloud, color: "#0ea5e9", desc: "Bridges FAI primitives to MCP and reports the tool surface discovered from the active runtime." },
...
function PackagesTab() {
  return (
    <div>
      <p style={{ fontSize: 13, lineHeight: 1.8, opacity: 0.85, marginBottom: 16 }}>
        <strong>FAI Packages</strong> deliver the same FAI contracts through VS Code, MCP, Python, Docker, CLI, and web surfaces. Availability and counts come from each active runtime instead of fixed marketing snapshots.
      </p>

      <Section icon={Package} title="Distribution Channels" color="#0ea5e9">
        <div style={{ display: "grid", gap: 8 }}>
          {[
            { name: "VS Code Extension", pkg: "frootai.frootai-vscode", ver: "Native", Icon: Cog, color: "#0ea5e9", desc: "IDE-native product system, catalogs, workflows, and evidence surfaces" },
            { name: "MCP Server (npm)", pkg: "frootai-mcp", ver: "Runtime", Icon: Cloud, color: "#10b981", desc: "Runtime-discovered tools for MCP-aware assistants" },
            { name: "Python SDK (PyPI)", pkg: "frootai", ver: "SDK", Icon: Package, color: "#f59e0b", desc: "Python-native access to Plays, primitives, and knowledge contracts" },
            { name: "Python MCP (PyPI)", pkg: "frootai-mcp", ver: "Runtime", Icon: Cloud, color: "#8b5cf6", desc: "Python MCP transport over the shared FAI contracts" },
            { name: "Docker Image", pkg: "ghcr.io/frootai/frootai-mcp", ver: "Image", Icon: Box, color: "#06b6d4", desc: "Containerized MCP runtime for cloud and CI/CD environments" },
            { name: "CLI", pkg: "frootai", ver: "CLI", Icon: Code, color: "#a16207", desc: "Command-line surface for discovery, validation, and delivery" },
          ].map(ch => (
