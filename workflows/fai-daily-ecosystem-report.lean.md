...
|-----------|---------|-------------|
| `competitor_agents` | 187 | Baseline agent count |
...
| `alert_validation_errors` | 0 | Max validation errors |
| `alert_stale_marketplace_hours` | 48 | Stale threshold for `marketplace.json` |
| `trend_comparison_days` | 1 | Trend lookback days |
...
Compute deltas vs competitor baselines:
...
Flag stale if `MARKETPLACE_AGE_HOURS > alert_stale_marketplace_hours`.
...
If `CHECKS_ERRORS > alert_validation_errors`, mark critical.
...
Use git log for the reporting period:
...
Find the latest daily report issue and compute deltas:

```bash
PREV_ISSUE=$(gh issue list --label "daily-report" --state closed --limit 1 --json body -q '.[0].body')
```

Extract prior primitive counts from the last report and compute deltas.
...
Always produce a report even with partial data. Indicate skipped sections.
...
If critical alerts exist (validation errors, security vulnerabilities, stale marketplace), append:
