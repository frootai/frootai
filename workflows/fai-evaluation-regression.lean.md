...
Read `fai-manifest.json` and extract `primitives.guardrails`.
...
Search result files per play in priority order.
...
Extract metric scores from the latest result file per play.
...
Compute delta/classification for each play-metric pair.
...
If no regression and scores improved, mark baseline for update.
